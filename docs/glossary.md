...
- **Workflow tool (Claude Code)** — a built-in execution *primitive*: a tool the model calls to orchestrate many sub-agents deterministically (`parallel()`/`pipeline()`, per-agent output schemas, a token budget, background execution + resume). oh-my-claude uses it as an **opt-in substrate** for heavy fan-out (council Phase 8 waves, large audits, migrations) when `workflow_substrate=on`. Don't confuse it with `/ulw` (a *mode*): the word "workflow" collides, the function does not — the Workflow tool is *how* heavy work fans out; `/ulw` is *how well* it gets done.
- **Workflow engine / graph-of-roles** — the paradigm where *every* prompt routes through a planner→researcher→implementer→reviewer state machine. Rejected as oh-my-claude's *default* paradigm (latency/cost, ULW-momentum) but **embraced as an opt-in substrate** now that Claude Code ships a Workflow tool (the original "no engine available" reason expired — see `docs/architecture.md` "Alternatives considered and rejected"). oh-my-claude stays a prompt-injected directive substrate that *calls* the engine rather than *becoming* one.
