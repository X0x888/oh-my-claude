...
| `quality_policy` | `balanced` | Adaptive quality posture. `balanced` preserves low-friction defaults for small/non-strict work. `zero_steering` records a prompt risk tier and keeps serious/high-risk work blocking after caps while keeping low-risk work compact. ULW's default `no_defer_mode=on` independently hard-blocks serious incompleteness after caps so autonomous execution does not end by narrowing or deferring scope. The `maximum` / `zero-steering` presets set this to `zero_steering`; balanced/minimal set it to `balanced`. Env: `OMC_QUALITY_POLICY`. |
...
The conf walk-up that lets a project-level `.claude/oh-my-claude.conf` override `~/.claude/oh-my-claude.conf` previously let any repo a user `cd`'d into disable defensive gates by committing a conf file. v1.43 introduces a narrow deny-list at the conf parser: **`pretool_intent_guard`, `bg_spawn_gate`, `agent_first_gate`, and `no_defer_mode` cannot be set via project-level conf.** Lines for those flags in project conf are silently ignored (the user-conf value and env-var override still work normally). All other flags continue to honor project conf as expected.
