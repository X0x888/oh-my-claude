...
|---|---|---|---|
| `quality` | All 34 | 0 | Users with high usage limits who prioritize quality over cost |
| `balanced` | 13 (planning, review, debugging, writing, operations) | 21 (execution, research, domain specialists) | Most users (default) |
| `economy` | 0 | All 34 | Users on tighter plans or budget-conscious API usage |
...
| `model_tier` | `balanced` | Agent model tier across all 34 specialist agents: `quality` = Opus everywhere, `balanced` = Opus for planning/review + Sonnet for execution, `economy` = Sonnet everywhere. **Two halves since v1.47**: the *agent-file half* is install-time — frontmatter `model:` lines are rewritten when `install.sh --model-tier=<tier>`, `switch-tier.sh <tier>`, or `/omc-config` runs (editing the conf line alone does not rewrite agent files) — and the *router half* is runtime — under `/ulw` the prompt-intent-router reads this flag fresh each prompt and injects a `SUBAGENT MODEL ENFORCEMENT` directive naming the `model` parameter to pass on every `Agent()` call, with the tool-call parameter authoritative over frontmatter (so a stale install-time rewrite cannot misroute dispatches). Project-level `.claude/oh-my-claude.conf` may set it — deny-list evaluation (v1.47): flipping it cannot disable an enforcement surface or cause data persistence; worst case is a per-repo cost/quality tier shift, visible in the injected directive and in transcripts, and env wins over both scopes. Env: `OMC_MODEL_TIER`. |
| `model_overrides` | _(empty)_ | Per-agent model override — the fine-grained complement to `model_tier`. Where `model_tier` is all-or-nothing across all 34 agents, this pins a specific model on a specific agent. Format is a comma-separated list of `agent:model` pairs; `model` is `opus`, `sonnet`, or `haiku`; `agent` is the bare agent filename without `.md` (e.g. `oracle`, `quality-reviewer`, `librarian`). Example: `model_overrides=oracle:opus,librarian:haiku`. Applied **after** the bulk `model_tier` rewrite, so an override always wins over the tier — the canonical use is `model_tier=economy` (cheap roster) plus overrides that lift the judgment-heavy agents (reviewer, planner, oracle) back to Opus while dropping a lookup agent like the librarian to Haiku. Unlike `model_tier` (which since v1.47 also has a runtime router half), this is an **install-time-only** flag (it takes effect when `install.sh` or `switch-tier.sh` runs, not by editing the conf line alone), and it survives a later `switch-tier.sh` call. Bad pairs (unknown model, or a missing agent file) are skipped with a warning, never fatal. Env: `OMC_MODEL_OVERRIDES` (takes precedence over the conf value). |
