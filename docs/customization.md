...
| `model_tier` | `balanced` | Agent model tier across all 37 specialist agents: `quality` = Opus everywhere, `balanced` = Opus for planning/review + Sonnet for execution, `economy` = Sonnet everywhere. **Two halves since v1.47**: the *agent-file half* is install-time — frontmatter `model:` lines are rewritten when `install.sh --model-tier=<tier>`, `switch-tier.sh <tier>`, or `/omc-config` runs (editing the conf line alone does not rewrite agent files) — and the *router half* is runtime — under `/ulw` the prompt-intent-router reads this flag fresh each prompt and injects a `SUBAGENT MODEL ENFORCEMENT` directive naming the `model` parameter to pass on every `Agent()` call, with the tool-call parameter authoritative over frontmatter (so a stale install-time rewrite cannot misroute dispatches). Project-level `.claude/oh-my-claude.conf` may set it — deny-list evaluation (v1.47): flipping it cannot disable an enforcement surface or cause data persistence; worst case is a per-repo cost/quality tier shift, visible in the injected directive and in transcripts, and env wins over both scopes. Env: `OMC_MODEL_TIER`. |
