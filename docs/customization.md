...
| `quality_policy` | `balanced` | Adaptive quality posture. `balanced` preserves low-friction defaults. `zero_steering` records a prompt risk tier and keeps serious/high-risk work blocking after caps while keeping low-risk work compact. The `maximum` / `zero-steering` presets set this to `zero_steering`; balanced/minimal set it to `balanced`. Env: `OMC_QUALITY_POLICY`. |
