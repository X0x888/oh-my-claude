...
| `statusline_width` | `on` | (v1.48-pre; conf home for the previously env-only `OMC_STATUSLINE_WIDTH`) Width-fit engine: when `on`, each statusline line sheds/shrinks its lowest-priority tokens **until it fits the terminal width** (a real fit computation — v1.48-pre replaced the v1.42.x fixed `<100`-col breakpoint, which left 100-157-col terminals with the full ~158-col worst case and never bounded the branch name). Line 1 shed order: drop `style:` → shorten `git:` to `b:` → truncate the branch name (ellipsis, ≥5 chars kept, dirty `*` preserved) → drop the `↑v` drift arrow. Line 2 shed order: drop `API:%` → `C:%` → `[gw:N]` → halve the context bar. Identity/mode tokens (`[Model]`, dir, `[ULW:]`, `[goal]`, `[g: f:]`, `v<installed>`) and the rate-limit tokens are never shed. A line that already fits renders in full at any width; unknown width (no `COLUMNS`, not a tty) renders everything. `off` = never shed. Same user-level-conf-only + display-only posture as `statusline_retention`. Env: `OMC_STATUSLINE_WIDTH` (wins over conf). |
