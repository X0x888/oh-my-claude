...
| `exhaustive_auth_directive` | `on` | (v1.46) Sibling to god-scope for **prose** open mandates. When `on`, a prompt explicitly authorizing exhaustive / open-ended improvement ("implement all improvements", "comprehensively evaluate and improve", "make it better", "fix everything" — `is_exhaustive_authorization_request`) injects an **OPEN-MANDATE / INNOVATION-GENERATION** directive at prompt time: treat the deliverable as the **delta to the most powerful version** (generate a wide improvement set), NOT a defect audit, and do not narrow an open mandate into a closeable subset. Closes the gap where prose open mandates (>30 chars) never reached god-scope's bare-verb path — the mandate-narrowing failure (`model-robustness.md` genuine-gap #4) where a model swaps an open improvement mandate for a closeable defect-audit and stops. **Non-blocking** (a directive, not a gate): the model first judges open-vs-scoped and honors an explicit narrow scope, so a fuzzy match on a scoped ask ("make the parser production-ready") costs only a mild nudge, never a false Stop-block — the blocking objective-contract gate deliberately does NOT consume this recall-tuned signal. `off` = strict prompt-literal scoping. Env: `OMC_EXHAUSTIVE_AUTH_DIRECTIVE`. |
