...
| `goal_gate` | `on` | Master switch for the `/goal` relentless driver (Codex `/goal` port). When on, a user-declared goal re-anchors at every Stop and blocks until achieved (fresh audit + `**Goal achieved.**`) or a no-progress stuck-wall. Opt-in — inert until `/goal` is run. |
