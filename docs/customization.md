...
| `guard_exhaustion_mode` | `scorecard` | Behavior when a gate's block cap is reached. `scorecard` (default) emits a quality scorecard then releases the stop for non-strict cases; `block` keeps blocking; `silent` releases without output. Under ULW execution with `no_defer_mode=on`, serious missing work (missing review/verification, failed or low-confidence proof, unremediated findings, pending scope, or session-handoff language) keeps blocking after caps unless strict autonomy is explicitly disabled. Legacy aliases accepted: `warn` → `scorecard`, `strict` → `block`, `release` → `silent`. Env: `OMC_GUARD_EXHAUSTION_MODE`. |
...
The conf walk-up that lets a project-level `.claude/oh-my-claude.conf` override `~/.claude/oh-my-claude.conf` previously let any repo a user `cd`'d into disable defensive gates by committing a conf file. v1.43 introduces a narrow deny-list at the conf parser: **`pretool_intent_guard`, `bg_spawn_gate`, `agent_first_gate`, `no_defer_mode`, and (v1.47) `quality_policy` cannot be set via project-level conf.** Lines for those flags in project conf are silently ignored (the user-conf value and env-var override still work normally). All other flags continue to honor project conf as expected.
