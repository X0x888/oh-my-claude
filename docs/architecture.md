...
**`skills/autowork/scripts/common.sh`** (~2,850 lines, with two subsystems extracted to `lib/`) -- Every hook script sources this file. State I/O lives in `lib/state-io.sh` (extracted in v1.12.0); prompt classification lives in `lib/classifier.sh` (extracted in v1.13.0). Both libs are sourced from `common.sh` so callers see a single import surface. Together they provide:
