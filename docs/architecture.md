...
~/.claude/quality-pack/state/<SESSION_ID>/
  ...
  timing.jsonl                 # Per-call timing rows appended by pretool-timing.sh / posttool-timing.sh / prompt-intent-router.sh / stop-time-summary.sh (lock-free, sub-PIPE_BUF O_APPEND). Row shapes (discriminated by `kind`): {kind:"prompt_start", ts, prompt_seq}, {kind:"prompt_end", ts, prompt_seq, duration_s}, {kind:"start", ts, tool, prompt_seq, tool_use_id?, subagent?}, {kind:"end", ts, tool, prompt_seq, tool_use_id?}, {kind:"directive_emitted", ts, prompt_seq, name, bytes} (per-directive byte cost emitted by prompt-intent-router.sh's add_directive helper for /ulw-report and offline cost-attribution analysis). Read by stop-time-summary.sh's `timing_aggregate` for the Stop epilogue, by show-time.sh for /ulw-time, and by /ulw-status. Fast-path-skipped when `time_tracking=off`. Aggregator silently ignores unknown kinds via the reduce `else .` branch — directive_emitted rows do not perturb start/end pairing.
```
