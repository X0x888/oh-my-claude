...
| `task_risk_tier` | Adaptive risk tier for the current ULW prompt: `low`, `medium`, or `high`. Written by `prompt-intent-router.sh` from prompt/domain/intent signals and consumed by strict/zero-steering gates to decide when scorecard release is too weak. |
