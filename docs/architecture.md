...
~/.claude/quality-pack/state/<SESSION_ID>/
  ...
  resume_request.json          # Written by stop-failure-handler.sh when Claude Code emits StopFailure: {rate_limited, matcher, original_objective, last_user_prompt, resets_at_ts, project_key, rate_limit_snapshot, ...}. project_key (added Wave 1) is the git-remote-first / cwd-hash _omc_project_key for the cwd at write time — used by Wave 1's resume-hint hook to match worktrees and clone-anywhere paths. Read by session-start-resume-hint.sh on every SessionStart (Wave 1 of the auto-resume harness — surfaces a hint until claimed). Will also be read by future Wave 2 `/ulw-resume` skill (atomic claim) and Wave 3 watchdog (autonomous re-launch).
