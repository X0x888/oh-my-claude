...
**`skills/autowork/scripts/common.sh`** (~7,200 lines, with five subsystems extracted to `lib/`) -- Every hook script sources this file. State I/O lives in `lib/state-io.sh` (extracted in v1.12.0); prompt classification lives in `lib/classifier.sh` (extracted in v1.13.0); verification scoring (Bash + MCP) lives in `lib/verification.sh` (extracted in v1.14.0); timing capture + the `/ulw-time` card live in `lib/timing.sh`; model-drift confabulation-canary computations live in `lib/canary.sh` (v1.26.0). All five libs are sourced from `common.sh` so callers see a single import surface. Together they provide:
...
**`skills/autowork/scripts/posttool-dispatch.sh`** -- **Universal-matcher PostToolUse dispatcher (v1.48 W3.1).** Replaces four per-call hook processes with one: sources `common.sh` once, then pipeline-subshell-sources `posttool-timing.sh` for every tool and additionally `record-verification.sh` → `record-delivery-action.sh` → `circuit-breaker.sh` for Bash calls. Handler scripts stay byte-identical and still run standalone (the `mcp__.*` matcher invokes `record-verification.sh` directly); their `. common.sh` lines short-circuit via the idempotent re-source guard at the top of `common.sh`; their `exit`s terminate only their own subshell. Circuit-breaker is the only emitting handler and runs last with stdout passed through unmerged. Cuts ~3× the self-documented ~25-30ms per-process cold-start tax on every Bash call. Regression net: `tests/test-posttool-dispatch.sh`.
...
**FINDINGS_JSON parser** (in `lib/state-io.sh` callers, helpers live in `common.sh`) -- Two helpers were added in v1.28.0 for the structured reviewer-finding contract: `extract_findings_json(message)` parses a `FINDINGS_JSON: [...]` block from the tail of an agent message (preferring `^FINDINGS_JSON:` anchor, requires `[` to validate), strips fenced code blocks, and emits one canonical JSON row per finding via `jq -c`; `normalize_finding_object(row)` coerces severity aliases (`critical`/`p0`/`p1`/`p2`/`blocker` → `high`/`medium`/`low`), normalizes unknown categories to `other`, caps `claim` to 140 chars and `evidence`/`recommended_fix` to 600 chars each, and emits a `_finding_id` for de-duplication. `extract_discovered_findings` takes the JSON path when present and emits rows with a `.structured` field carrying the full payload, falling through to the legacy prose-heuristic path only when the JSON line is missing/empty/malformed (fail-open by design). Multi-line / pretty-printed arrays are also accepted — the parser captures from the last unindented `FINDINGS_JSON:` line through the closing array (or the `VERDICT:` line); single-line form is merely preferred for grep/debug workflows. The contract-presence regression net is `tests/test-findings-json.sh`. Emitting agents: `quality-reviewer`, `excellence-reviewer`, `oracle`, `abstraction-critic`, `metis`, `design-reviewer`, `briefing-analyst`, `release-reviewer` (added v1.32.1).
...
~/.claude/quality-pack/state/<SESSION_ID>/
  ...
  timing.jsonl                 # Per-call timing rows appended by pretool-timing.sh / posttool-timing.sh / prompt-intent-router.sh / stop-time-summary.sh (lock-free, sub-PIPE_BUF O_APPEND). Row shapes (discriminated by `kind`): {kind:"prompt_start", ts, prompt_seq}, {kind:"prompt_end", ts, prompt_seq, duration_s}, {kind:"start", ts, tool, prompt_seq, tool_use_id?, subagent?}, {kind:"end", ts, tool, prompt_seq, tool_use_id?}, {kind:"directive_emitted", ts, prompt_seq, name, chars} (per-directive size in locale-aware codepoint count — emitted by prompt-intent-router.sh's add_directive helper for /ulw-report and offline cost-attribution analysis; field is `chars` not `bytes` because `${#var}` in bash is locale-aware and returns codepoints under UTF-8, which correlates with token cost better than raw bytes anyway), {kind:"token_delta", ts, prompt_seq, main_in, main_out, main_cache_read, main_cache_creation, agent_in, agent_out, agent_cache_read, agent_cache_creation} (v1.46-pre — per-Stop token usage parsed from Claude Code's session transcript by stop-time-summary.sh's `timing_capture_session_tokens`, split main-thread vs sub-agent via the transcript's `isSidechain` flag; an incremental cursor `token_transcript_rows` in session_state.json prevents double-counting across Stops; emitted only when `token_tracking=on`). Read by stop-time-summary.sh's `timing_aggregate` for the Stop epilogue, by show-time.sh for /ulw-time, and by /ulw-status. Fast-path-skipped when `time_tracking=off`. Aggregator silently ignores unknown kinds via the reduce `else .` branch — directive_emitted rows do not perturb start/end pairing.
```
...
| `outcome` | enum | Stop-hook-set values: `completed` (all gates satisfied) \| `released` (clean exit, no gates fired — advisory pass / no-edits) \| `skip-released` (user invoked `/ulw-skip` to bypass; gates fired but were NOT model-resolved — DO NOT count as caught) \| `exhausted` (guard cap hit). Sweep-inferred values (v1.39.0 W1): `completed_inferred` (no Stop hook ran but `last_review_ts` + `last_verify_ts` are set AND `code_edits>0` — strongest positive signal recoverable post-hoc) \| `idle` (zero edits AND no review/verify — sweep saw no work activity) \| `unclassified_by_sweep` (partial signals — neither clearly shipped nor clearly idle). Live-session synthesizer defaults to `active`. The prior `abandoned` default was retired because it mislabeled ~348/418 historical rows that were really idle or unclassified. |
...
| `last_verify_scope` | Verification scope label written by `record-verification.sh`: `targeted`, `full`, `lint`, `build`, `operations`, `unknown_test`, `unknown`, or `mcp_<check_type>`. Delivery Contract v2 uses it so lint/build/browser proof cannot suppress missing-test inference for changed code. |
...
| `goal_last_block_edit_ts` | `last_edit_ts` at the previous goal block (the progress detector). |
...
| `objective_contract_audited_ts` | **v1.46-pre Codex /goal port.** Epoch ts written when the model emits an explicit `**Objective coverage.**` attestation in the closing region (detected via `has_closeout_label coverage`). While `> objective_contract_prompt_ts` the cycle is considered audited and the gate stays quiet; reset to empty on the next fresh execution prompt. Note: because the cycle re-stamps only on fresh EXECUTION intent (not continuation), a continuation turn that adds new scope to an already-audited cycle stays quiet — a deliberate under-block consistent with the precision-over-recall posture (re-issue as a fresh execution prompt to re-arm), not a bug. |
...
| `token_transcript_rows` | **v1.46-pre token tracking.** Incremental cursor (row count) of how much of Claude Code's session transcript `timing_capture_session_tokens` (lib/timing.sh) has already accounted for. At each Stop only rows beyond this cursor are parsed and summed into a `token_delta` timing row, so every transcript usage row is counted exactly once. Reset to the current row count when the transcript shrinks (compaction rewrite). Written only when `token_tracking=on`. Stored as a string (the bulk `read_state_keys` reader cannot frame raw JSON numbers). |
| `bg_work_dispatched_ts` | **v1.46-pre background-dispatch awareness.** Set by `posttool-timing.sh` when a backgrounded **Bash** command's `.tool_response` carries the `running in background with ID:` marker (other background tools rely on the behavioral "Waiting on background work" announcement, not this marker; detection is gated by `time_tracking`). Read+CONSUMED single-shot by `stop-guard.sh` **at the top, before any early-return path** (non-ULW / `stop_hook_active` / gate-skip exits): when present it sets the in-process `_OMC_BG_WORK_PENDING_NOTE` flag so `format_gate_block_dual` appends a "waiting, not stopped" note to any block message (message-only — the block/release decision is unchanged), then clears the key so the note cannot stale into a later turn. Consuming ahead of every exit is the correct single-shot expiry: a recency gate (`>= last_user_prompt_ts`) would never expire because the background-completion notification re-invokes the model as a synthetic prompt that does NOT advance `last_user_prompt_ts`. |
...
| `session_outcome` | How the session ended. State-key values written by Stop hooks (v1.34.2 enum, unchanged): `completed` (all gates satisfied — strongest positive signal), `released` (clean exit without firing any gates — advisory pass / no-edits / first-prompt release), `skip-released` (user explicitly invoked `/ulw-skip` to bypass; gates fired but were NOT model-resolved — downstream trust surfaces MUST exclude these from "caught + resolved" claims), `exhausted` (guard caps reached after N blocks; gates fired and remained unsatisfied). v1.39.0 W1 retired the bare `abandoned` sweep default — when the daily sweep encounters session state without a Stop-hook write, it now infers `completed_inferred` (reviewed+verified+edits>0), `idle` (zero activity), or `unclassified_by_sweep` (partial signals). See `outcome` row in the `session_summary.jsonl` table above for the full sweep-inferred value-space. Carried into `session_summary.jsonl` for cross-session analytics. |
