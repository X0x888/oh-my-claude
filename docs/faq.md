...
12. **"I'm blocked on a real operational input the agent can't supply (credentials, login, infra down, rate limit hit, destructive shared-state action awaiting confirmation, unfamiliar in-progress state)."** → `/ulw-pause <reason>` — declares an **operational** pause without tripping the session-handoff gate. Distinct from `/ulw-skip` (gate bypass) and `/mark-deferred` (legacy soft-defer, refused under ULW execution with default `no_defer_mode=on`) — this signals "I'm paused, waiting for an INPUT only the user can supply on X". v1.40.0 narrowed the criterion: taste/policy/brand-voice/credible-approach are no longer pause cases — under ULW the agent picks the sane default with stated reasoning and ships. Cap is 2 pauses per session; the pause flag clears at the next user prompt.
...
### Does Council always dispatch the same agents on a large task?

No. `/council` builds a coverage map from the actual question and repository evidence, then considers the full available agent roster. The normal primary team is one to four specialists, selected with explicit include/skip reasons and dispatched concurrently; one agent is valid when the question is genuinely narrow. Four is a normal envelope, not a hard ceiling: Council may exceed it only when the persisted map evidences more than four mutually independent, high-impact competence needs that cannot be combined safely, and it must name the added cost. After reconciling every primary return, Council may run one optional gap-fill round of zero to two agents for a concrete uncovered question, then sends at most three load-bearing findings to independent verifiers whose competence matches the claim.

`--polish` and `--self-audit` raise particular coverage needs, but do not force a named panel. `--deep` escalates selected Sonnet-backed agents to Opus and leaves `model: inherit` agents on the main session model. The normal call envelope is therefore primary 1–4 + conditional gap-fill 0–2 + verification 0–3, while focused councils are usually smaller and an evidenced exceptional primary can be larger. `council_coverage.json` is a locked lifecycle rather than a static list: initialization is primary-only; recorded primary returns plus structured reconciliation are required before up to two gap-fill mandates can appear; final reconciliation and every selected return are required before the assessment handoff becomes ready. Namespaced agent identities are exact, and unnamespaced installed names bind once to the runtime identity that uses them. `[council:primary]`, `[council:gap-fill]`, and `[council:verification]` are telemetry/provenance tags for those phases—not prefixes that select a standing set of agents. Broad whole-project assessments can auto-trigger Council under `/ulw`; a focused feature, capability, surface, or subsystem should use the narrower route unless the prompt explicitly requests Council.

A successfully completed, fully reconciled advisory assessment leaves a single-use handoff for the immediately following prompt. A matching follow-up such as “implement the recommendations” can enter Phase 8 using the assessment's coverage and findings instead of dispatching the panel again; any intervening prompt expires the handoff. That authorizes the assessed scope only. Full-scope expansion is decided by `is_exhaustive_authorization_request()`—explicit totality forms, exhaustive implementation wording, high-bar targets, and binary-quality framing—not by thorough assessment wording; otherwise the normal scope-expansion pause applies.

### Does editing several files always force the same reviewer chain?

No. Code and prose still receive their universal reviewer. Design review needs a UI-shaped change **and** evidence that design judgment is material: UI/visual intent, explicit broad scope, a current complex plan, or an active Phase 8 wave. Editing business logic in a `.tsx` or native view file does not by itself force design review on a fresh objective. UI detection includes web components/styles/markup, storyboards/XIBs, high-confidence Swift view/screen/controller names and directories, Android `res/layout` XML, and high-confidence Kotlin/Compose UI names and directories; tests and generated/minified styles are excluded.

Completeness is added when the objective is explicitly broad/open, has a current complex plan or active wave plan, has unknown Bash edit scope, or is cross-surface at the configured breadth. Traceability requires both its file threshold and cross-surface/current-plan/active-wave evidence. Metis remains a plan-phase stress test. A fresh execution prompt snapshots the edit-log offset plus Bash, plan, and findings-ledger generations, so old objective work does not inflate the next review while a repeat edit still counts. Reviewer dispatches and Bash/MCP verification calls capture their relevant generation at start; an edit during either operation rejects that result, and missing/replayed/malformed tool snapshots fail closed rather than guessing from completion order. Reviewer state is isolated by role, so a clean specialist cannot satisfy or erase the generic review.

### Are source documents and native Office artifacts tracked the same way?

No. Source-form writing/research files such as `.tex`, `.bib`, `.typ`, `.qmd`, and `.rmd` are path-classified as prose, so exact-path edits enter the editor-critic route. That classification does not prove the source compiles or renders; use the project's real TeX/Typst/Quarto/R Markdown build or rendered-output check when applicable.

Connector-managed `.docx`, `.pptx`, and `.xlsx` files are binary artifacts and may be written through tools that do not expose an exact path to the ordinary Edit/Write hooks. The document/spreadsheet/presentation workflow must therefore verify the saved artifact through its native connector/render/recalculation loop; do not infer hook-level edit/review coverage merely from the file's existence. A shell command whose mutation is observed still advances conservative unknown-Bash coverage, but it cannot invent the exact binary surface.
...
Agent permission boundaries are a safety mechanism. The 26 inspection/judgment agents deny Claude Code's direct mutation tools with `disallowedTools: Write, Edit, MultiEdit, NotebookEdit` and request `permissionMode: plan`. They keep Bash because inspection and verification often require `git diff`, tests, or builds, so this is not an OS-level no-write sandbox: Bash remains subject to the active parent permission mode, which can override a subagent's plan mode in `acceptEdits`, `auto`, or bypass configurations. The 11 domain-builder agents (`frontend-developer`, `backend-api-developer`, the `ios-*` family, `research-data-analyst`, etc.) deliberately retain editor tools because implementation is their job.
...
3. **Headless watchdog (opt-in)** — a LaunchAgent (macOS) / systemd user-timer (Linux) / managed crontab entry (fallback) polls every ~2 minutes; when the rate-limit window clears, atomically claims the artifact and launches `claude --resume <sid> '<original prompt>'` in a detached `tmux` session. Falls back to an OS notification when `tmux` is unavailable. Enable via `/omc-config` (set `resume_watchdog=on` and the helper installs the platform scheduler, including a managed cron entry when `crontab` is available).
...
- **Balanced** — close to install-time defaults; tighter on a few quality knobs without the cost of quality-tier Opus execution.
...
### I turned on `intent_verify_directive` (or `prometheus_suggest`) and the model proceeded without asking — is that a bug?

No, that is the v1.24.0 contract working as designed. Both flags were originally written (v1.19.0) to inject a directive that told the model to "ask the user to confirm or correct" / "restate the goal before its first edit" — which under ULW produced a hold pattern ("The advisory gate flagged this turn as classification-ambiguous, so I'm holding before edits.") that violated the core ULW rule (the user's request IS the permission). v1.24.0 rewrote both directives to **declare-and-proceed**: when a short, ambiguous, or product-shaped execution prompt arrives, the model states its interpretation in one or two declarative sentences as part of its opener (e.g., "I'm interpreting this as &lt;X&gt; and proceeding now"), then starts work. You can interrupt and redirect in real time if the call is wrong; the directive's job is to make the model's interpretation **auditable**, not to **block** forward motion. Pause case is narrow: only when both confidence is low AND the wrong call would be hard to reverse (the credible-approach-split test from `core.md`). The flag names and defense purpose are unchanged — only the model-facing wording. If you genuinely want the v1.19.0 ask-and-hold behavior, you'll need to add custom system-prompt overrides; the bundled flags no longer carry that contract. See `core.md` "Veteran default for ambiguous prompts: declare-and-proceed, never ask-and-hold."
