...
Verification is tied to the code generation at **tool start**, not completion. The PreToolUse `record-tool-start-revision.sh` hook snapshots `last_code_edit_revision` in a tool-use-ID-keyed `.verification-starts/` sidecar for Bash/MCP calls. `record-verification.sh` consumes that sidecar and commits accepted `last_verify_*` state under the same lock only when tool identity matches and the code generation is unchanged. Missing, malformed, mismatched, replayed, or edit-during-verification evidence fails closed and records `last_stale_verify_*` diagnostics without erasing prior accepted proof. Keep this path independent of optional timing telemetry; disabling `time_tracking` must never weaken verification freshness.
