...
2. **One question, only if useful.** Default to model tier `balanced` unless the user has already expressed a preference. Don't ask if they shrug or are unfamiliar — just use the default. Suggested phrasing if you do ask:
   ...
   > Default model tier is `balanced` (planning/review agents ride your session's main model, Sonnet for ordinary execution, with high-risk escalation). Want the Opus-execution `quality` posture or the Sonnet-first adaptive `economy` posture instead?
...
oh-my-claude/
  ...
  bundle/dot-claude/          # Installs to ~/.claude/
    ...
    skills/                   # 36 skill definitions, each in <name>/SKILL.md
      autowork/scripts/       # 51 autowork hook scripts and utilities
        ...
        lib/plan-publication-transaction.sh # Planner multi-artifact rollback WAL + cold rebind handoff
...
`N=0` should be rendered as `CLEAN`/`SHIP`, never `FINDINGS (0)` — though the parser accepts and re-maps it to clean for safety. The stop-guard's `record-reviewer.sh` hook reads this line to determine whether to tick the reviewer's dimension. For bundled current tracked calls, the first two missing, misplaced, or wrong-role verdicts continue from the exact native agent's retained context; neither parallel SubagentStop hook may consume its causal rows or publish partial evidence. A third invalid return retires the call and makes the parent dispatch a fresh equivalent. If a hard runtime limit skips continuation, foreground calls recover at Agent PostTool, while background calls recover from the native-ID task notification; Stop-level live-registry reconciliation remains the final backstop against a passive wait. Phrase-based detection exists only for untracked pre-contract migration sessions and external plugin reviewers that own a different output contract.

Reviewer state is role-isolated. Only `standard` code review writes `last_review_ts`, `review_had_findings`, and the generic `review_format_issue`; prose writes `last_doc_review_ts` plus `doc_review_had_findings`, and completeness, stress-test, traceability, design, and release reviewers write only their own timestamps/verdict dimensions. A specialist completion therefore cannot satisfy the universal code-review gate or erase a standard review's findings. `record-pending-agent.sh` snapshots the relevant edit/plan generation and ULW enforcement interval at dispatch; its `SubagentStart` mode binds Claude Code's platform-issued `agent_id` to the exact pending/start row and commits that binding in `native_agent_bindings.jsonl`. `record-reviewer.sh` rejects a result if the native binding, generation, or stable objective identity changed before completion and commits accepted metadata/dimensions in one locked batch. The universal `record-subagent-summary.sh` independently checks the same frozen generation before any summary, discovered-scope, Council-evidence, uncertainty-credit, or accepted-outcome side effect, so either SubagentStop hook order rejects stale evidence consistently. After exact identity/objective/generation validation but before any completion claim, it also applies the bounded retry policy above; foreground/background hard-limit recovery shares that counter. The dedicated recorder exits silently, leaving both causal rows unconsumed and intact apart from the bounded counter until a complete retry or transactional retirement. Every cleanup-only ignored outcome is committed first under the session lock as a versioned roll-forward journal containing separate exact pending/start fingerprints. Pre-commit failure leaves both rows intact and advertises no replacement; after commit, interruption may leave either row, but foreground/background outcome consumers plus replay/admission paths reconcile only those exact fingerprints before consuming the journal or offering/admitting replacement work. Accepted results append objective/revision-scoped structured evidence using the consumed start row, never completion-time state. History never ticks a dimension. Confirmed killed/interrupted, prior-objective, or parent-hard-limit-exhausted tombstone reuse requires a unique `[review-rebind:<id>]` authorization token; a third-invalid SubagentStop retirement removes both rows and admits an ordinary fresh equivalent. Current native-hook clients bind the replacement automatically; the echoed `REVIEW_DISPATCH_ID: <id>` immediately before a valid universal `VERDICT` remains the compatibility path for an in-flight session created before native tracking was armed. `record-subagent-summary.sh` durably claims the exact pending row through all effects and stores a redacted digest-bound replay message. The original publisher owns a 120-second lease; after strict expiry the row remains a mutation fence while exact replay takes over lifecycle-idempotent effects. Effects-complete remains fenced until the accepted parent outcome and exact pending/waiter settlement are durable. Current sessions fail closed if the native binding, replay authority, or pending row is missing. Abandoned returns are cleared without summaries, scope, gates, uncertainty evidence, dimensions, Council provenance, or historical reflection reuse. Council Phase 8 prefixes every selected reviewer/semantic-risk description in a settled wave with `[review-batch]`; the intent guard freezes mutation only for live current-objective rows and ignores settled abandonment tombstones. This is cooperative same-user process state rather than an OS security boundary: a process that can rewrite the session directory can tamper with the ledgers. The causal contract prevents ordinary replay, out-of-order completion, and model-output forgery without maintaining a permanent semantic-role list.

Current dispatch rows also carry a harness-generated immutable `lifecycle_dispatch_id`. Cleanup journal v2 stores that ID as `cleanup_lifecycle_dispatch_id` in addition to the exact byte fingerprints, so legitimate post-commit abandonment/claim metadata rewrites remain recognizable without broad role/native matching. A duplicate lifecycle ID is ambiguous and fails closed; unresolved versioned journals are not age-pruned.

Reviewer publication is crash-atomic. `record-reviewer.sh` stages its exact causal-row removals, owned state patch, review history, Definition evidence/frontier artifacts, and lifecycle/native/completion-digest decision receipt in the fixed `.reviewer-transaction.wal`; recovery rolls that generation forward idempotently and publishes the receipt last. Summary-first returns publish no universal effect before that receipt and leave `reviewer_summary_waiters.jsonl`; exact receipt-bound replay retains an effects-complete pending row until the accepted parent outcome is durable, then consumes pending and waiter in recoverable order. Every outer session-state lock settles fixed planner/reviewer authority before mutation (publisher recovery and exact reset have narrow internal bypasses), so prompts, sibling callbacks, Agent admission, PostTool writes, compaction/resume, and Stop cannot observe or overwrite a provisional generation. Any malformed/symlinked journal fails closed; only the exact managed `/ulw-off` command may quarantine it without dereferencing the node.
...
2. **Gate-side enforcement (`stop-guard.sh` stricter-verdict-wins gate).** After the review-coverage gate confirms all required dimensions are ticked, the stricter-verdict-wins gate scans each required dim's verdict and blocks Stop on any fresh `FINDINGS*`/`BLOCK*`. Freshness is dimension-specific and revision-first: code compares with `last_code_edit_revision`, design with `last_ui_edit_revision`, prose with the latest relevant doc/unknown-Bash edit revision, completeness/traceability with `edit_revision`, and stress-test with `plan_revision`. Timestamp clocks are migration fallbacks only.
...
Nine finding-emitting agents — `quality-reviewer`, `excellence-reviewer`, `release-reviewer`, `oracle`, `abstraction-critic`, `metis`, `design-reviewer`, `briefing-analyst`, `rigor-reviewer` — emit a single-line `FINDINGS_JSON: [...]` block in the terminal metadata block when findings exist. It immediately precedes the optional `REVIEW_DISPATCH_ID` and final `VERDICT`, except that an `excellence-reviewer` operating against a current Definition of Excellent places the mandatory `QUALITY_REVIEW_JSON` between `FINDINGS_JSON` and those final lines. Each finding object: `{severity, category, file, line, claim, evidence, recommended_fix}`. Severity ∈ `{high, medium, low}`. Category ∈ `{bug, missing_test, completeness, security, performance, docs, integration, design, other}`. Empty array `[]` is valid for clean reviews.
...
Check 4 of stop-guard derives required dimensions from the **current execution objective's** changed surfaces and semantic breadth. Fresh execution prompts snapshot the current `edited_files.log` line offset plus Bash-event, plan-revision, findings-signature, and prompt-time semantic/broad-scope markers; continuations preserve them. Later unique log rows define the current path set, so re-editing a path counts while prior-objective rows do not leak in. Generic code/prose review follows its surface. UI-shaped paths require UI/visual semantics or broader complex/active-wave evidence before design review; completeness and traceability are selected only when applicable rather than from a standing reviewer sequence.

| Reviewer | Dimension(s) ticked |
|---|---|
| `quality-reviewer` | `bug_hunt`, `code_quality` |
| `metis` | `stress_test` (plan-phase only; never forced by post-edit file count) |
| `excellence-reviewer` | `completeness` for broad, qualifying cross-surface, open-mandate, current-complex-plan, active Phase 8 wave-plan, or unknown-scope work (plus `last_excellence_review_ts`) |
...
| `briefing-analyst` | `traceability` (requires both `traceability_file_count`+ current-objective files and cross-surface, current-complex-plan, or active-wave scope) |
| `design-reviewer` | `design_quality` when a recognized UI-shaped edit is paired with UI/visual semantics or broader complex/active-wave scope. Recognition includes web components/styles/markup, storyboards/XIBs, high-confidence Swift UI names/directories, Android `res/layout` XML, and high-confidence Kotlin/Compose UI names/directories; tests and generated/minified styles are excluded. |
...
Each dimension ticks via `tick_dimension <name>` in `common.sh`, under the state lock. `dimension_freshness_revision` is canonical for new state: code → `last_code_edit_revision`, design → `last_ui_edit_revision`, prose → the latest relevant doc/unknown-Bash revision, completeness/traceability → `edit_revision`, stress-test → `plan_revision`. `dimension_freshness_clock` supplies timestamp fallback for migrated sessions. A relevant post-tick change implicitly invalidates the dimension even when edit and review share an epoch second. Unknown Bash scope conservatively selects generic code quality plus completeness; route-time prose/UI semantics may add those reviewers without fabricating an exact path or traceability breadth. Source-form `.tex`, `.bib`, `.typ`, `.qmd`, and `.rmd` paths classify as prose, but still need their project-specific compile/render verification when applicable.

When adding a new reviewer-style agent that should participate in adaptive coverage, wire it in `config/settings.patch.json` under `SubagentStop` with the reviewer-type argument: `$HOME/.claude/skills/autowork/scripts/record-reviewer.sh <type>` where `<type>` is `standard`, `excellence`, `prose`, `stress_test`, `traceability`, or `design_quality`. Update the selector, the dimension freshness mapping, this table, and the reviewer's agent file in lockstep.

#### Causal verification freshness

Verification is tied to the work generation at **tool start**, not completion. The PreToolUse `record-tool-start-revision.sh` hook snapshots work/edit/code/plan/review-cycle/Definition coordinates in a tool-use-ID-keyed `.verification-starts/` sidecar for Bash, Read/Grep, and MCP calls. `record-verification.sh` consumes that sidecar and commits accepted state/receipts under the same lock only when tool identity and every causal coordinate still match. The first Definition envelope seals the current verification threshold and additive revisions inherit it, so later policy changes neither rescue nor invalidate that proof language. Definition Bash receipts reject compound/discovery/skip/zero-test executions and derive identity from the harness-observable semantic target: opaque custom-script argv, unknown/nonsemantic runner options, path aliases, shell-launch spelling, timing, and claimed evidence-kind labels cannot manufacture independent proof, while allowlisted framework selectors remain part of the target. Freeze-time Bash witnesses rank structured runner targets above explicitly pathed custom verifiers above bare commands; different equal-rank targets are invalid. Read/Grep proof binds one existing canonical in-project target. Current Playwright snapshot and screenshot proof both bind the exact classifier plus case-sensitive `target=<selector-or-ref>`. `observed_url` is snapshot-only because screenshot output cannot reliably expose route identity. Query and fragment text remain snapshot route identity, and frozen URLs require lowercase scheme/host with no default port or dot path segment. Contract anchors declare raw JSON scalar values; the harness creates receipt/witness authority by escaping `%` to `%25` before `;` to `%3B`, preserving raw literal `%3B` versus raw literal `;` and preventing field injection. Control bytes are rejected rather than stripped, and a value requiring secret redaction is unavailable proof. Source and connector aliases therefore cannot split one observation. A successful benchmark/comparison/render must also emit a kind-specific positive result; silence, generic PASS prose, process failure, or zero observation persists current negative evidence instead of reusing an older green receipt. Missing, malformed, mismatched, replayed, edit-during-verification, or non-executing evidence fails closed without erasing prior accepted proof. Keep this path independent of optional timing telemetry; disabling `time_tracking` must never weaken verification freshness.

#### Discovered-scope capture (advisory specialists)

Separately from the reviewer dimensions above, the universal SubagentStop hook (`record-subagent-summary.sh`) captures findings into per-session `discovered_scope.jsonl`. Prose heuristic capture is allowlisted to `metis`, `briefing-analyst`, `oracle`, `abstraction-critic`, `editor-critic`, `rigor-reviewer`, and the seven Council lenses. Any other installed/custom specialist can enter the safer deterministic path by emitting valid `FINDINGS_JSON`. Verifier agents (`quality-reviewer`, `excellence-reviewer`, `design-reviewer`, `release-reviewer`) are excluded from the generic structured path because their findings already feed dedicated dimensions. `prometheus` prose remains excluded because it returns plans, not findings.
...
**Council selection is broader than the prose whitelist.** Adaptive Council may select any installed/custom agent whose competence matches a coverage need. Require `FINDINGS_JSON` from non-allowlisted finders; do not broaden heuristic prose parsing merely to make an agent selectable.

**Council coverage is a mechanical lifecycle.** Before dispatching an Agent whose description begins `[council:primary]`, initialize `council_coverage.json` through `record-council-coverage.sh init`. Generation 1 requires at least one primary and zero gap-fill selections; the normal primary envelope is 1–4, while exceeding four requires an explicit `primary_exception` that names the cost and every independent high-impact coverage ID. After all recorded primary returns, `update` preserves the objective and existing coverage rows verbatim, requires row-by-row reconciliation, and may add at most two gap-fill selections tied to partial, uncovered, or newly discovered coverage. `primary-complete` requires every selected result `evidenced`; unresolved/new coverage must take the sole gap-fill round. After selected gap-fill returns, a final `gap-fill-complete` update is required and may retain explicit residual limitations. Verification is optional: zero is valid; each of at most three unique exact `[council:verification]` identities requires a current final-reconciled, not-yet-completed ledger and carries objective/generation provenance. `complete` requires final reconciliation, every selected return, and no current in-flight primary/gap/verification dispatch, and is the only producer of the revision-stamped `council_assessment_ready` handoff. Every material coverage row needs evidence, impact, required competence, and status; every selection needs exact identity, non-overlapping coverage IDs, reason, and non-goals. A later Council prompt may `init` a fresh map and archive the prior one; same-prompt concurrent initialization is lock-serialized so exactly one succeeds.

**Phase 8 entry is not exhaustive authorization.** Only `record-council-coverage.sh complete` on a fully returned, reconciled advisory Council records a handoff, and only the immediately following matching prompt revision may reuse its coverage/findings without recreating a panel; any intervening prompt consumes it. The router stamps `council_phase8_prompt_revision`, and completion suppresses a redundant handoff only when that stamp equals the completed objective's revision. Bare `implement` or `ship` enters Phase 8 for the assessed scope. Only `is_exhaustive_authorization_request()` permits all recorded/new scope to proceed without the normal scope-explosion pause.

**Binary connector residual.** Connector-managed `.docx`, `.pptx`, and `.xlsx` writes may not expose an exact path to Edit/Write hooks. Do not claim ordinary edit-clock or dimension coverage from file existence alone; native artifact workflows must render/recalculate/inspect the saved artifact through their connector-specific verification loop.

**Dual-pipeline participation.** `metis` and `briefing-analyst` participate in **both** the reviewer-dimension pipeline (via `record-reviewer.sh stress_test` / `traceability` matchers) and the discovered-scope ledger (via the universal summary hook). These are independent surfaces for an accepted, current-generation completion: Metis' tick is plan-phase, briefing's traceability tick is selected only for applicable current-objective breadth, and the discovered-scope ledger preserves any findings either produces. A generation-stale completion is rejected by both paths and creates neither a tick nor scope debt. Accepted findings do not deduplicate across the two surfaces — when adding or modifying these specialists, update both surfaces deliberately. The same applies to any future advisory specialist that also acts as a verifier.

**Silent-disarm telemetry.** When a prose-allowlisted specialist returns more than 500 characters but the heuristic extractor catches zero findings, `record-subagent-summary.sh` writes a `[anomaly]` row via `log_anomaly`. This makes prose-style drift (specialist drops a `### Findings` heading, switches to prose-only output) visible instead of silently disabling the gate.
...
- Agent `model:` assignments (`inherit`, `opus`, or `sonnet`) are set in the bundle but can be overridden via `--model-tier` and `model_overrides`. When adding a new agent, assign `model: inherit` for complex reasoning tasks (rides the current session model without hard-coding a temporary model name) or `model: sonnet` for faster execution tasks; reserve a hard `model: opus` pin for cases that must not float with the session. The live `/ulw` resolver combines the declaration with tier, reasoning risk, Council deep mode, and explicit overrides. See [customization.md](docs/customization.md#model-tiers) for details.
