...
oh-my-claude/
  ...
  bundle/dot-claude/          # Installs to ~/.claude/
    ...
    skills/                   # 34 skill definitions, each in <name>/SKILL.md
      autowork/scripts/       # 44 autowork hook scripts and utilities
  ...
  tests/                      # 147 bash + 1 python test scripts; CLAUDE.md "Testing" gives the canonical commands
...
Reviewer state is role-isolated. Only `standard` code review writes `last_review_ts`, `review_had_findings`, and the generic `review_format_issue`; prose writes `last_doc_review_ts` plus `doc_review_had_findings`, and completeness, stress-test, traceability, design, and release reviewers write only their own timestamps/verdict dimensions. A specialist completion therefore cannot satisfy the universal code-review gate or erase a standard review's findings. `record-pending-agent.sh` snapshots the relevant edit/plan generation at dispatch; `record-reviewer.sh` rejects a result if that generation changed before completion and commits accepted metadata/dimensions in one locked batch. Because `SubagentStop` has no dispatch ID, the PreTool hook denies a duplicate in-flight gate-reviewer role; distinct roles can remain parallel without ambiguous FIFO attribution.
...
**Dual-pipeline participation.** `metis` and `briefing-analyst` participate in **both** the reviewer-dimension pipeline (via `record-reviewer.sh stress_test` / `traceability` matchers) and the discovered-scope ledger (via the universal summary hook). These are independent surfaces: Metis' tick is plan-phase, briefing's traceability tick is selected only for applicable current-objective breadth, and the discovered-scope ledger preserves any findings either produces. They do not deduplicate — when adding or modifying these specialists, update both surfaces deliberately. The same applies to any future advisory specialist that also acts as a verifier.
