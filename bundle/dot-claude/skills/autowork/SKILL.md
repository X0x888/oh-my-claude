...
5. Before finalizing, run the universal review for each edited surface (`quality-reviewer` for code, `editor-critic` for prose), then add only the specialist coverage justified by the current objective's surfaces and semantic risk. `metis` stress-tests plans; `briefing-analyst` covers traceability-heavy analysis.
...
12. For semantically broad tasks (cross-surface changes, open mandates, multi-wave work, or uncertain edit scope), after the universal review pass run `excellence-reviewer` for a fresh-eyes holistic evaluation of the full deliverable — completeness, unknown unknowns, and polish opportunities that defect-focused review misses. When dispatching it, include the original task objective and any scope items identified during planning so the reviewer has a concrete checklist. File count is a breadth floor, not sufficient reason by itself; skip excellence for focused changes whose required surfaces are already covered.
...
- For heavy fan-out — a `/council` Phase 8 wave-batch, a large multi-file audit, a migration across many files — Claude Code's **Workflow tool** (deterministic multi-subagent orchestration: `parallel()`/`pipeline()`, per-agent output schemas, a token budget, resume) is an opt-in execution substrate when `workflow_substrate=on`; `/ulw` is standing authorization for the tool's explicit-opt-in precondition. It is NOT the default per-prompt path: small and ad-hoc work stays on the lightweight in-thread path (the latency/cost and momentum reasons behind `docs/architecture.md`'s graph-of-roles rejection still hold — only the "no engine available" premise lifted). Reach for it on observable heavy shapes, never on a feeling that a small change is "big." Doctrine: `model-robustness.md` Mechanisms 2–3.
...
- A reviewer's / critic's / sub-agent's FINDING or BLOCK is an input, not an automatic verdict. Default to acting on it — but ground acceptance in the cited evidence (re-read the source the finding names; confirm it reproduces), and reject only via the evidence-checked `not reproducible`/`false positive` path (`record-finding-list.sh`). This is not license to discount findings (that inverts into the anti-finding bypass v1.42.x F-010 closed); it is the requirement that compliance be evidence-grounded, not reflexive. Doctrine: `intellectual-craft.md` Popper #6 (symmetric half), `model-robustness.md` Mechanism 8. This applies to reviewer/critic/sub-agent verdicts ONLY — NOT to a stop-guard or mechanical gate block, which stay under `model-robustness.md` Mechanism 4 (do not reason around the gate; the `BLOCK MODE` token is not a "verdict to challenge").
...
4. The changed files have been re-read with fresh eyes (by the universal reviewer and any surface/risk-relevant specialist) and any findings are shown in auditable form.
