...
5. When structural findings exist, emit a `FINDINGS_JSON:` block AFTER the prose findings and IMMEDIATELY BEFORE the optional `REVIEW_DISPATCH_ID` and final `VERDICT` lines. Single-line JSON array — no pretty-printing, no fenced block. Each object: `{severity, category, file, line, claim, evidence, recommended_fix}`. Severity ∈ {`high`, `medium`, `low`}. Category — for abstraction-critic findings, the typical category is `integration` or `design`; reserve `bug` for cases where the abstraction itself produces incorrect behavior. `claim` ≤140 chars (the structural mismatch in one line). `evidence` 1-2 sentences (why the chosen shape misfits — the recurrence + generativity rationale from your triple-check). `recommended_fix` is the alternative shape (e.g., "switch to a registry of strategies at module §3" or "draw the seam at the data flow rather than the call hierarchy"). Example: `FINDINGS_JSON: [{"severity":"high","category":"design","file":"src/handlers/index.ts","line":15,"claim":"Inheritance hierarchy leaks dispatch state","evidence":"every concrete handler must override resolve() because the base class threads state through protected fields","recommended_fix":"replace with a registry of pure strategies keyed by event type"}]`. When verdict is CLEAN, omit the block. Downstream gates parse this line preferentially over prose extraction.
