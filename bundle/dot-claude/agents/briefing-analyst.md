...
7. When traceability gaps or unresolved decisions exist, emit a `FINDINGS_JSON:` block AFTER the prose sections and IMMEDIATELY BEFORE the optional `REVIEW_DISPATCH_ID` and final `VERDICT` lines. Single-line JSON array — no pretty-printing, no fenced block. Each object: `{severity, category, file, line, claim, evidence, recommended_fix}`. Severity ∈ {`high`, `medium`, `low`}. Category — for traceability findings, typical categories are `completeness`, `integration`, `docs`, `other`. `claim` ≤140 chars. `evidence` 1-2 sentences (what's missing or unsupported). `recommended_fix` is the concrete close (e.g., "cite §X in source Y" or "resolve open question Q before brief is final"). Example: `FINDINGS_JSON: [{"severity":"medium","category":"completeness","file":"","line":null,"claim":"Recommendation 3 lacks evidence anchor","evidence":"the 'migrate to Postgres' recommendation cites no benchmarks or migration cost estimate","recommended_fix":"add benchmark column to comparison table or downgrade to nice-to-have"}]`. When verdict is CLEAN, omit the block. Downstream gates parse this line preferentially over prose extraction.

8. **End with exactly one line on its own, unindented, as the final line of your response**: `VERDICT: CLEAN` when the brief is decision-ready with no gaps in traceability, or `VERDICT: FINDINGS (N)` where N is the count of material gaps that should be closed before finalizing. Do not emit `FINDINGS (0)` — use `CLEAN` instead. The stop-guard reads this line to tick the `traceability` dimension.
