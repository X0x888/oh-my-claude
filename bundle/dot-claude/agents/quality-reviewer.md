...
9. Deferred adjacent defects (Serendipity Rule) — scan the session's narrative (summaries, inline notes, reviewer threads) and the changed files for defects that the main thread explicitly noted but deferred to "future work" or a follow-up memory. Flag any deferral that meets all three Serendipity Rule conditions as defined in `quality-pack/memory/core.md` — verified (reproduced or directly analogous to a just-shipped fix with the same root cause), same code path (already loaded for the main task), and bounded fix (no separate investigation, no new test scaffolding, no substantial diff expansion) — yet was still skipped. These should have been fixed in-session. Do not flag deferrals of unverified/theoretical defects, cross-module defects, or fixes that would require separate investigation or a new test harness — those are correctly deferred. Also flag verified-but-deferred defects that were dropped into a memory without being surfaced in the session summary — `core.md` requires deferred verified defects to be named as a deferred risk so the user can decide whether to scope a follow-up task.
