...
7. Verify the top of the stack: pick the 2-3 highest-impact findings and re-dispatch \`oracle\` per finding to verify each claim against the actual code before presenting. Mark each as ✓ verified, ◑ refined, or ✗ demoted/dropped. Cap at 3."
      ...
      if is_execution_intent_value "${TASK_INTENT}"; then
        _council_phase8_hint="
8. Execute the assessment (Phase 8). Step 7's presentation is NOT the finish line for this prompt — the user asked for implementation.${_council_authorization_hint} **Resume check first:** run \`record-finding-list.sh counts\` — if a wave plan already exists with pending findings, do NOT re-bootstrap (init refuses by default; --force would clobber progress). Re-enter at the in-progress wave instead. **Otherwise bootstrap** the master finding list with stable IDs (F-001, F-002, ...): \`echo '[{\"id\":\"F-001\",\"summary\":\"...\",\"severity\":\"critical\",\"surface\":\"...\",\"effort\":\"S\"}, ...]' | record-finding-list.sh init\` (script auto-discovers the active session). When initing, **include \`requires_user_decision: true\` and \`decision_reason: \"...\"\` on findings that need user input** (taste / policy / brand-voice / credible-approach split). For findings discovered post-bootstrap, run \`record-finding-list.sh mark-user-decision <id> <reason>\` to flag them. Then \`record-finding-list.sh assign-wave <idx> <total> <surface> F-xxx F-yyy ...\` per wave, and \`record-finding-list.sh status F-xxx shipped <commit-sha> '<notes>'\` after each fix. **If a wave reveals a new finding** that was missed, append it via \`record-finding-list.sh add-finding <<< '{\"id\":\"F-NNN\",\"summary\":\"...\",\"severity\":\"...\",\"surface\":\"...\"}'\` and assign-wave it; do NOT silently fix outside the master list. **Wave grouping:** aim for 5–10 findings per wave by surface area; if your plan emerges with avg <3 findings/wave on a master list of ≥5 findings, that is over-segmentation — merge adjacent surfaces until each wave is substantive. Execute each wave fully (quality-planner → implementation specialist → quality-reviewer on the wave's diff → excellence-reviewer for the wave's surface → verification → per-wave commit titled 'Wave N/M: <surface> (F-xxx, ...)'). **Pause on USER-DECISION findings:** when a wave contains a finding with \`requires_user_decision: true\`, surface that finding's summary + decision_reason to the user before executing the wave (or before that specific finding's fix within the wave). Do NOT choose autonomously on user-decision findings — that is the rule's purpose. Resume the wave once the user has weighed in. Do NOT clip scope to the five-priority headline (that rule is presentation-only); do NOT collapse waves into one mega-commit; do NOT defer waves to a future session. If the prompt did NOT explicitly authorize exhaustive implementation (no canonical Phase 8 entry marker fired — the marker list is in council/SKILL.md Step 8 and the bash predicate \`is_exhaustive_authorization_request()\` mirrors it), surface the wave plan first and apply the Scope explosion pause case from core.md. Full Phase 8 protocol is in the council skill definition. Final summary: run \`record-finding-list.sh summary\` for the markdown finding-status table — USER-DECISION findings appear in their own column AND are surfaced separately for visibility."
      fi
