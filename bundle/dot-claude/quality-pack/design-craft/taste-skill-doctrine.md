<!--
oh-my-claude design-craft reference — taste-skill doctrine
...
design-reviewer, frontend-developer, frontend-design SKILL, ios-ui-developer, and
the UX-trimmed design-lens). It is NOT in the global @-include chain — read it
on demand when the design-craft surfaces calibrate.

Update by re-pulling from upstream main; do not hand-edit individual rules
without recording the divergence here. The MIT license preserved alongside
this directory (or in attribution comments) permits commercial use, modification,
and distribution with attribution.

The headline mechanism is the three numeric dials (DESIGN_VARIANCE / MOTION_INTENSITY
/ VISUAL_DENSITY, 1-10 each) plus the em-dash ban (§9.G). The framework-locked
sections (§5 React/Motion/GSAP code skeletons, §12 BLOCK LIBRARY) are preserved
verbatim but readers on other stacks can skip them.
-->
