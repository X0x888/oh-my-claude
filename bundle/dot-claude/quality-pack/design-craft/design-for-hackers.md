<!--
oh-my-claude design-craft reference — Design-for-Hackers method
...
lookup + Anti-Rationalization Table) is the load-bearing lift. The cited
chapters ("Ch. 1", "Ch. 3", etc.) reference David Kadavy's *Design for Hackers*
(2011), which the upstream skill paraphrases as background reading; the skill's
own checklists and gates are the original authored material we are vendoring.

This is an on-demand reference loaded by 6 design-side surfaces. Update by
re-pulling from upstream main; do not hand-edit individual rules without
recording the divergence here.
-->
