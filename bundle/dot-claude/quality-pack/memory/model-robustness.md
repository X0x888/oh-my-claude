...
## When The Mechanisms Run Out — The Three Genuine Gaps

Three model failure modes do **not** have a structural mechanism in the current harness. They are documented here so the model knows where its judgment is the only line of defense — *and* so future paradigm-level work has named targets.
