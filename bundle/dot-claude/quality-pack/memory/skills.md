...
- `/council [focus] [--deep]` dispatches a multi-role project-evaluation panel (PM, design, security, data, SRE, growth). Now includes a Phase 7 verification pass that re-checks the top 2-3 findings via `oracle` before presenting. `--deep` escalates lenses to opus for higher-stakes audits (cost: meaningfully higher per run). Also auto-triggers under `/ulw` when the prompt asks for a broad project assessment.
