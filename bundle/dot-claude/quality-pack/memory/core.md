...
- **Background-process hygiene: clean up before stopping.** If you spawn a long-running background process during a session — a dev server, an `asciinema rec`, a `watch` loop, a `tmux new-session`, an `npm run dev`, anything started with `&` / `nohup` / `setsid` / `run_in_background: true` — track its PID and clean it up before stop. The user observed real recurring damage: detached `omc-resume-*` tmux sessions left by the resume-watchdog accumulated 4+ orphans over 8–11 days, each carrying a stuck `claude --resume` process. The harness now auto-cleans those specific orphans via the SessionStart `cleanup-orphan-resume.sh` hook (default-on; threshold `orphan_resume_max_age_hours=4`), but the safety net only covers `omc-resume-*` sessions — your responsibility for everything else you spawn. Specifically:
    - **Foreground commands by default.** Don't reach for `&` or `run_in_background: true` unless the work genuinely outlives the current command.
    - **Track every PID you spawn.** Capture `$!` after `&`, the foreground task ID for `run_in_background: true`. Don't fire-and-forget.
    - **Kill before stopping.** Before declaring the task done, terminate any process you spawned: `kill "$pid"`, `tmux kill-session -t <name>`, stop the dev server, end the asciinema recording. The end-of-task summary line should *name* what was killed.
    - **Diagnostic prompt:** before stop, run `ps -ef | grep -i "<your-command>"` (or `tmux ls` for tmux work) to confirm nothing you started is still alive. If it is, kill it or explicitly justify why it should survive (e.g., the user asked you to start a dev server they'll use after the session).
