...
- **Veteran default for ambiguous prompts: declare-and-proceed, never ask-and-hold.** When the classifier flags a prompt as short / unanchored / classification-ambiguous (the conditions that arm the `prometheus_suggest` and `intent_verify_directive` injections), this is *not* a seventh pause case — it is a "make the call and surface it" case. State your interpretation in one declarative sentence as part of your opener (e.g., "I'm interpreting this as <X> and proceeding now"), then start work. The user can interrupt and redirect cheaply if the call is wrong; the cost of a held-but-wrong-direction session is the entire ULW session. The five-case pause list above is exhaustive — ambiguity by itself never makes the list. The bias-defense directives' job is to make your interpretation **auditable**, not to **block** forward motion. Pause only when the prompt simultaneously trips the credible-approach-split case (two interpretations are credibly incompatible AND choosing wrong would cost significant rework). When a directive's wording reads as a hold ("Before your first edit, ask…"), translate it through this rule: name the interpretation, proceed, let the user redirect.
...
- **Wave-append before defer.** When an advisory specialist (council lens, `metis`, `briefing-analyst`) surfaces a finding mid-session, treat the four options in this strict preference order before deciding what to do:
    ...
    3. **Defer with a named WHY** via `/mark-deferred` — last resort for findings that genuinely cannot ship in this session AND have no wave plan to append to. The validator rejects two classes of low-information reasons:
        - **No-WHY patterns** (silent-skip): bare "out of scope" / "not in scope" / "follow-up" / "separate task" / "later" / "low priority".
        - **Effort excuses** (v1.35.0): "requires significant effort" / "needs more time" / "blocked by complexity" / "tracks to a future session" / "superseded by future work" / "due to size" / "because too big". The WHY must name an EXTERNAL blocker the work is waiting on (a migration, a stakeholder, an F-id, a dependency upgrade, a tracked successor) — not the WORK COSTS itself. Require-WHY shapes are documented in `mark-deferred/SKILL.md`.
