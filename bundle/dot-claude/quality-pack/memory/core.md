...
- **The "too heavy for this session" rationalization is the failure mode `/ulw` was built to prevent — not a legitimate stop signal.** Real reported case (v1.40.x): a session at ~33% context closed with *"Remaining heavy refactors queued from the original v12 plan… These are multi-hour each with UI-render verification requirements… A fresh /council pass would also surface post-v12 findings cleanly. Both candidates for next session."* Each clause is a self-justifying rationalization that the agent owns the technical judgment on — and used to escape work instead of own it. The contradiction the rationalization is trying to dissolve is real: long-context drift IS a thing, fresh perspective IS sometimes valuable, multi-hour work IS legitimately effortful. But "stop and defer to next session" is the wrong resolution. The right resolutions, by rationalization shape, are:
...
- **The session-handoff gate is a backstop, not the standard.** As of v1.40.x it catches preposition-anchored handoff phrasings — `for|to|in|until` followed by `(a|the|another)?` followed by `next/future/later/separate session` — in addition to the earlier `new session` / `another session` / `next wave` / `next phase` patterns. (Bare `fresh session` was excluded after a false-positive audit — ambient harness text uses "this fresh session" / "as a fresh session" / "do not treat as a fresh session" in install banners and compact directives; the model echoing those would block-storm.) The standard is recognizing the rationalization before the gate has to fire. The gate exists because the rationalization is hard to resist; the goal is to make the gate redundant by always preferring chunk-and-ship or sub-dispatch.
- **Hygiene: clean up after yourself.** If you spawn or create *any* transient artifact during a session — a background process, a temp file or directory, a recording, a scratch fixture, a detached service — track it and clean it up before stop. The user observed multiple instances of this failure mode in the wild:
    - **Background processes / detached shells.** 4 detached `omc-resume-*` tmux sessions left by the resume-watchdog accumulated over 8–11 days, each carrying a stuck `claude --resume` process at 25–51 CPU-minutes. Auto-swept by `cleanup-orphan-resume.sh` SessionStart hook (default-on; `orphan_resume_max_age_hours=4`).
    - **Temp files and directories.** 56 `/tmp/omc-sterile-tmp-*` dirs accumulated from `tests/lib/sterile-env.sh:build_sterile_env` because the subshell-capture pattern (`sterile_env="$(build_sterile_env)"`) hid the created paths from the parent shell's EXIT trap. Auto-swept by `cleanup-orphan-tmp.sh` SessionStart hook (default-on; `orphan_tmp_max_age_hours=24`); source-side fix in `tests/run-sterile.sh` parses HOME and TMPDIR via `extract_sterile_path` and traps both for cleanup.
    - **The class is broader than the two examples above.** Recordings (`asciinema rec`), dev servers (`npm run dev`), watchers, `nohup` / `setsid` / `tmux new-session`, any `&`-detached shell, any `run_in_background: true` Bash invocation, any `mktemp -d` without trap, any download/scratch directory the agent created — all the same class.
    The auto-cleanup hooks are the *safety net*, not the standard. The standard is the agent recognizing what it spawned/created and cleaning it explicitly. Concretely:
    ...
    - **Helpers that create scratch artifacts should expose cleanup symmetry.** When you write a helper that creates state (a `setup_foo` function, a `build_bar` script), expose a matching `cleanup_foo` and document the caller responsibility. If the helper hides its output behind a `$()` capture pattern, ALSO expose an extractor (the v1.40.x `extract_sterile_path` pattern in `tests/lib/sterile-env.sh` is the worked example).
    ...
    - **The harness can't sweep what it doesn't own.** Both cleanup hooks are pattern-scoped (`omc-resume-*` and `/tmp/omc-*`). Artifacts outside those prefixes (a `/tmp/my-test/` dir, a non-`omc-` prefixed scratch file) are *yours* to clean.
