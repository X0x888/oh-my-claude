...
- **Waiting ≠ stopping — make verified in-flight work visible.** Use the ⏳ auto-resume line only while Claude Code's current registry reports a live background task (or compatible scheduled wake). SubagentStop may request a bounded same-transcript continuation for a missing required verdict, but a foreground Agent completion or background task notification has ended that runtime attempt even if its prose says “Let me…”. Resume the retained native agent or explicitly rebind immediately; do not wait for another notification from an ended task. For a verified wait, name what's awaited, that it auto-resumes, and that no user action is needed. Do not poll or spawn a waiter; the registered task's completion notification is the mechanism. The task-notification router and managed Stop dispatcher automatically recover incomplete/dead waits; the legacy `bg_work_dispatched_ts` note is only an old-client/live-registry backstop.
