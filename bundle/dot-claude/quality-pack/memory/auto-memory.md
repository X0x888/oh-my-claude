...
**Repo-shared complement (`repo_lessons=on`, off by default).** Everything above writes to `~/.claude` — machine-local, never committed. When `repo_lessons=on` (user-level conf or `OMC_REPO_LESSONS=on` only; project-conf cannot enable it), durable project lessons ALSO go to the repo itself via `bash ~/.claude/skills/autowork/scripts/record-repo-lesson.sh add lesson "..."` (or `add backlog "..."` for a follow-up item) — a team-shareable, git-committable bullet at `.claude/lessons.md` / `.claude/backlog.md`, capped at 7 per file, that survives across machines and teammates through normal git history instead of staying trapped on this one machine. The same non-derivability bar from "Reject these patterns" below still applies — a repo lesson is worth the write only when it is not already recoverable from `git log`, `CHANGELOG.md`, or the source tree. Never write secrets, credentials, tokens, or anything the user would not want in a commit — these bullets land in a file the user may `git add` and push.
