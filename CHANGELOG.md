...
- **`tests/test-design-contract.sh` (52 assertions, regression net).** Asserts the 9 canonical Stitch section names appear across all four surfaces (8/9 in design-reviewer where the 9th lens is "DESIGN.md drift"); ≥10 brand-archetype names live in frontend-design SKILL.md; the no-clobber clause (`never auto-write/create`) appears in skill, agent, and router; the opt-out detector regex includes the documented tokens; all three scope tiers (A/B/C) are present in both frontend-developer and the router (so a future edit can't silently drop Tier B/C from the most-frequently-fired surface); the differentiation directive ("three things" + "differently") is asserted in the router as a spirit-form anti-anchoring check; the VERDICT instruction line on both modified agent files is preserved as the final non-blank line; and `bash -n` succeeds on the router.
...
- **Router UI hint trimmed (~1200 → ~1000 chars).** The brand-archetype name list and the VoltAgent reference URL moved out of the router's user-visible context-injection string into the agent file (the user does not see agent prompts directly). The router still carries the 9-section anchor, all three scope tiers, the differentiation directive ("three things differently"), DESIGN.md awareness, the no-clobber rule, anti-patterns, and opt-out tokens — so first-time users get the routing guidance without the "lecturing about a thing called Stitch" effect that bloats the transcript.

### Deferred

- **Drift lens for inline-emitted contracts.** The `design-reviewer`'s 9th lens fires only when `DESIGN.md` exists at project root — but the auto-flow defaults to **inline** contract emission for first-time users (no project-root file). Result: lens 9 doesn't fire for the most common path. Two correct-coherent fixes (a) extend the lens to read inline contract blocks from `subagent_summaries.jsonl`, or (b) write a session-scoped `<session>/design_contract.md` that the reviewer reads alongside any project-root `DESIGN.md`. Out of scope for the current release; flagged so it is not silently dropped.
