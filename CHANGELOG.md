...
- `test-settings-merge.sh` (now 151 tests): updated SubagentStop count from 8 to 10; added name assertions for `metis`, `briefing-analyst`, and `editor-critic` matcher commands. Added four new scenario blocks:
  ...
  - **Cross-impl structural diff** (14 fixtures) — previously only hook counts were compared cross-impl, which could miss a divergence like "py: 1 hook, jq: 2 hooks" at the same count. Now runs 14 non-trivial fixtures through both impls and asserts byte-for-byte equality of `jq -S . py` vs `jq -S . jq`, including three-same-matcher-entry migration and embedded-newline command fixtures.
- Zero regressions across all 8 test suites (665 tests total, all passing).
