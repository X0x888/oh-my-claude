...
**Port 3 — Zero-finding evidence anchor rule in `quality-reviewer.md`.** New priority 12 in the agent prompt: if no defects are found, cite ≥2 concrete file:line evidence anchors in the **Summary** section proving the reviewer read the diff. If 2 anchors cannot be produced, downgrade to `VERDICT: FINDINGS (1)` with a meta-finding `{severity:"low", category:"other"}` requesting deeper read. Defends against the rubber-stamp-CLEAN failure mode the historical `unknown ×247` cluster surfaces. Prompt-side rule only — measurement via `/ulw-report` over time, not mechanical gate (decision in port-wave memory file). Ported from `romiluz13/cc10x/plugins/cc10x/agents/code-reviewer.md` zero-finding gate. Regression: `tests/test-quality-reviewer-zero-finding-rule.sh` (7 assertions).
