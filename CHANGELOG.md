...
**Deep-evaluated 10 high-star public Claude Code / AI-coding projects across 5 user-named domains** (iOS, web, design/aesthetic, extension, general coding), shipping the ports that closed real gaps without softening load-bearing contracts. Sources verified at the file level (not summary level) via `gh api` + `curl raw` reads — three claimed mechanisms were rejected once verified: (1) `southleft/figma-console-mcp` 6-gauge scorecard exists only in README marketing, not in the source code (which delegates to axe-core for a11y and Figma manifest extraction for tokens); (2) `senlindesign/claude2figma` PostToolUse "hook" is literally an `echo "QA: Verify..."` reminder string, not verification logic; (3) `sickn33/antigravity-awesome-skills/skills/design-taste-frontend/SKILL.md` is a downstream redistribution of `Leonxlnx/taste-skill` — used the MIT upstream directly instead.

**Port C — Three new vendored design-craft references.** Added `bundle/dot-claude/quality-pack/design-craft/taste-skill-doctrine.md` (Leonxlnx/taste-skill v2, MIT — anti-slop dials, em-dash ban, Jane Doe / fake-perfect-number / startup-slop-name bans, italic-descender clearance, §9.F production-test bans like decorative status dots and fake screenshots; ~73KB verbatim including the framework-locked React/Motion/GSAP code skeletons — the headline mechanism is the three numeric dials `DESIGN_VARIANCE`/`MOTION_INTENSITY`/`VISUAL_DENSITY` 1-10 and the em-dash ban which is the single most-violated LLM design tell); `design-for-hackers.md` (ryanthedev/design-for-ai, MIT — 10-category CHECKER + 8-phase APPLIER + Symptom→Chapter lookup + Anti-Rationalization Table, all cited to David Kadavy's *Design for Hackers*); `a11y-doctrine.md` (fecarrico/A11Y.md, MIT — POUR framework under WCAG 2.2 AA / ISO 9241-171 / ADA / EAA, 4-level severity matrix with MUST/SHOULD/MAY-FIX semantics, AI Behavior Contract, anti-patterns like Clickable Divs and Leaked Focus Traps, Definition-of-Done). Each file ships with a vendoring HTML-comment header naming upstream source, MIT license, vendoring date, and no-hand-edit / re-pull rule — mirrors the swiftui-pro pattern from Port B.

**Per CLAUDE.md "Adding or removing a design-craft reference" lockstep, all 4 sites updated:** (1) the three new files in `design-craft/`; (2) `verify.sh` `required_paths` registers each; (3) each of the **5 visual-craft consumer surfaces** (`visual-craft-lens`, `design-reviewer`, `frontend-developer`, `ios-ui-developer`, `frontend-design` SKILL) now has an `## Additional design-craft references (on-demand)` section listing all 3 references with on-when-to-load guidance scoped to that surface; (4) the **UX-trimmed consumer** (`design-lens`) has an `## Additional design-craft reference (on-demand)` section that references **ONLY `a11y-doctrine.md`** — the other two are visual-craft scope and the existing test-art-taste-doctrine.sh scope-discipline check (visual principles appear exactly once in design-lens, in the scope-disclaimer paragraph) still passes (99/99 assertions). Regression: `tests/test-design-craft-additions.sh` (53 assertions, CI-pinned). Count drift updated: 126 bash tests → 127.

**Port E — Document `ccusage` and `XcodeBuildMCP` as recommended companion tools in README.** Added a `### Recommended companion tools` subsection under `## Power-user setup` that documents two independent MIT-licensed tools that pair well with oh-my-claude: (a) [`ryoppippi/ccusage`](https://github.com/ryoppippi/ccusage) (14.7k stars, native Rust binary distributed via npm) — parses Claude Code conversation logs and aggregates token usage and cost against a locked LiteLLM pricing snapshot. The 5-hour `ccusage blocks` aggregation matches Anthropic's actual rate-limit window, which is the same surface oh-my-claude already tracks in `rate_limit_status.json`; `ccusage statusline` is a beta path for embedding in Claude Code's statusline. (b) [`getsentry/XcodeBuildMCP`](https://github.com/getsentry/XcodeBuildMCP) (5.7k stars, getsentry-maintained) — the same MCP recommended inline from the 4 iOS agents in Port A. Centralizing it in the README makes it discoverable to users browsing the docs rather than the agent files. Documentation-only — no bash port of ccusage's 5-hour-window logic (the Rust binary path is the better install vector; bash port would duplicate non-trivial work and lose the embedded LiteLLM snapshot). The deep-eval verified both repos are active, MIT, and self-distributing.

**Port D — Document VoltAgent `tools:` allowlist convention in CONTRIBUTING.md.** Updated `## Adding Agents` step 3 to describe both permission-boundary conventions: the **allowlist** (`tools: Read, Grep, Glob, ...`) — preferred for new security-sensitive agents because it fails closed when Claude Code adds new tools — and the existing **denylist** (`disallowedTools:`) — looser default, retained for existing agents. The allowlist is the documented Claude Code sub-agent spec and is also the convention adopted by [`VoltAgent/awesome-claude-code-subagents`](https://github.com/VoltAgent/awesome-claude-code-subagents) (20.5k stars). Also added a "Permission-boundary convention" subsection under "Reviewer-agent additions" with a concrete frontmatter example for new reviewer agents. Documentation-only change — no existing agents migrated. Existing reviewers using `disallowedTools:` (e.g., `quality-reviewer.md`) remain supported; the convention shift is for *new* agents, not a migration mandate. Verified the `model:` per-agent frontmatter is the Claude Code official spec — not a VoltAgent novelty — by reading 5 sample agents under `VoltAgent/awesome-claude-code-subagents/categories/`.

**Port B — Vendored `swiftui-pro` bundled skill (model-invoked SwiftUI reviewer).** New `bundle/dot-claude/skills/swiftui-pro/` directory: `SKILL.md` + `LICENSE` + 9 partial-loadable references (`api.md`, `accessibility.md`, `data.md`, `design.md`, `hygiene.md`, `navigation.md`, `performance.md`, `swift.md`, `views.md`). All content lifted verbatim from `twostraws/SwiftUI-Agent-Skill` v1.1 (MIT, © 2026 Paul Hudson — the *Hacking with Swift* author) — bundled-from / bundled-source-url metadata preserves provenance; vendoring comment forbids hand-edit and instructs re-pull. The skill is model-invoked (no `disable-model-invocation` in frontmatter), so Claude Code auto-loads it when SwiftUI work is in play; partial-load support means a prompt like "check for deprecated API" loads only `references/api.md`. The api.md reference is the LLM-distinctive lift — it names ~17 SwiftUI rules the model has been observed to violate from training-data memory (`foregroundStyle()` not `foregroundColor()`, `clipShape(.rect(cornerRadius:))` not `cornerRadius()`, `Tab` not `tabItem()`, `topBarLeading` not `navigationBarLeading`, the `@Entry` macro for `EnvironmentValues` keys, etc.) — exactly the surface the `ios-ui-developer` agent did not encode itself. Hudson is a recognized SwiftUI authority (Hacking with Swift); the MIT license explicitly permits vendoring with attribution. Per CLAUDE.md "Adding or removing a skill directory" lockstep: `verify.sh` registers SKILL.md + LICENSE + all 9 references; `uninstall.sh` `SKILL_DIRS` includes `skills/swiftui-pro`; `memory/skills.md` documents the model-invoked trigger; `README.md` skill table adds a row noting "no slash command — auto-fires on SwiftUI work". Regression: `tests/test-swiftui-pro-skill.sh` (33 assertions, CI-pinned in `.github/workflows/validate.yml`). Count drift in `README.md`, `AGENTS.md`, `CLAUDE.md` updated: 27 skills → 28; 125 bash tests → 126.

**Port A — XcodeBuildMCP recommendation in all 4 iOS agents.** Added `## Recommended companion MCP — XcodeBuildMCP` section to `bundle/dot-claude/agents/ios-ui-developer.md`, `ios-core-engineer.md`, `ios-deployment-specialist.md`, `ios-ecosystem-integrator.md`. Each block names the MIT-licensed Sentry-maintained MCP server (~5.7k stars, 82 manifest-driven tools grouped by workflow phase: build / test / sim / device / debug / SwiftPM / scaffold / UI-automation), provides install commands (`npx -y xcodebuildmcp@latest mcp` and `npm install -g xcodebuildmcp`), the MCP-config docs path, and the Sentry-telemetry privacy opt-out path. Recommendation rather than dependency — the iOS agents operate without it, but when a user is iterating SwiftUI previews against real simulators or building CI pipelines, the agent now has a documented executable-surface path rather than round-tripping through Bash. Sourced from `getsentry/XcodeBuildMCP` (MIT). Per-agent wording is calibrated to each agent's surface: deployment-specialist emphasizes CI/test pipelines, ui-developer emphasizes simulator/Dynamic Type/VoiceOver iteration, core-engineer is brief, ecosystem-integrator emphasizes hardware verification of entitlements.
