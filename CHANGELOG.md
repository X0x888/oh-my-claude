...
**Objective-completion contract (Codex `/goal` port).** New `objective-contract` stop-gate closes the long-standing "long/large tasks finish as a subset" failure mode. On a *substantive* execution turn it re-anchors the **verbatim original objective** + a completion audit before Stop, so the model cannot silently ship part of a multi-part objective and stop believing it is done. Every prior completion gate was *reactive* (deferral language, exemplifying markers, specialist-recorded findings); this is the first that holds the **primary objective's** scope. It borrows OpenAI Codex `/goal`'s durable re-anchored-objective idea but backs it with a **mechanical hard-block** — Codex's prose-only completion audit leaks through compaction (their issue #19910), whereas this re-runs from durable session state every Stop. Precision-calibrated: arms only on `plan_complexity_high` / per-cycle edit fan-out (`≥ objective_contract_min_files`) / long-objective signals, **self-disarms** on non-execution and follow-up turns (no turn-2 false positive), and **clears** on an explicit `**Objective coverage.**` attestation. The anti-premature-*stop* sibling of the v1.46-pre `/ulw-pause` 3-turn anti-premature-*give-up* gate; the first ship of the "completion contract" input-enrichment paradigm named in `model-robustness.md`. New flags `objective_contract_gate` (default `on`) + `objective_contract_min_files` (default `4`); directional false-positive observability wired into `/ulw-report` (`objective-contract` block-vs-reprompt rate). Known blind spot, documented not papered over: a short prose imperative implying large scope, done tiny, with no planner dispatch is beyond bash's mechanical reach — the gate stays silent there rather than false-block. Regression nets: `tests/test-objective-contract.sh` (the "fix our failure mode" arming probe + the turn-2-advisory false-positive probe) and F-014 in `tests/test-stop-guard-bypass-surface.sh` (end-to-end block + intent-flip resistance).

**Router redaction-fork dedup (latency).** `prompt-intent-router.sh` redacted the user prompt **twice** per prompt — once into `PROMPT_TEXT_SAFE` (the in-memory classifier-safe copy) and again into the persisted `_omc_persisted_prompt_safe` — with byte-identical input (`PROMPT_TEXT`, never reassigned) through the identical `omc_redact_secrets | tr -d '\000'` chain. The persist branch now reuses `PROMPT_TEXT_SAFE`, removing the router's heaviest *avoidable* per-prompt fork (a multi-pattern `sed -E` pipeline plus printf+tr) on every persist-on prompt (the default). Pure latency optimization, **zero behavior change**: the persisted redaction is proven byte-identical to the prior recompute across plain, secret-bearing (`--api-key sk-ant-…`, `Bearer …`), and `KEY=VALUE` prompts. The router remains fork-bound by its load-bearing classifier/directive work on bash-3.2-macOS (where `now_epoch`/`json_get` are forks, not builtins); deeper reduction is the deliberately-deferred `common.sh` split, not a bolt-on. Regression: `tests/test-prompt-router-latency.sh` (8 assertions, CI-pinned).

**Token-usage tracking (transcript-sourced).** The harness tracked where *wall-time* went (`/ulw-time`) but not where *tokens* went. New `token_tracking` (default `on`, sub-toggle of `time_tracking`) closes that: at Stop, `timing_capture_session_tokens` parses the rows appended to Claude Code's session transcript since the previous Stop and attributes input / output / cache-read / cache-creation tokens to the **main thread vs sub-agents** (via the transcript's `isSidechain` flag), appending a `token_delta` row to the same `timing.jsonl` ledger the time card uses. An incremental row cursor (`token_transcript_rows`) counts every usage row **exactly once** across Stops — robust against the transcript's many harness-injected `user` rows (task-notifications, Stop-feedback) that defeat content-based prompt-boundary detection. A compaction-rewrite guard resets the cursor when the transcript shrinks. Surfaces: a token line in `/ulw-time` (`tokens in 1.2M (89% cached) · out 340K · agents 45%`) and `/ulw-status`, and a **Token usage** section in `/ulw-report` (window total + main-vs-sub-agent table). Cost is bounded to the new transcript slice (~50ms on a 3MB transcript, cold-path once per Stop, fail-open) — it does not regress the Stop hook. **Scope:** token *counts* only — dollar cost stays on the statusline (Claude Code's authoritative `cost.total_cost_usd`) and [`ccusage`](https://github.com/ryoppippi/ccusage); per-*individual*-agent attribution (vs the main-vs-aggregate-agents split shipped here) is deferred. Token counts carry no prompt content (lower privacy surface than prompt persistence). Conf flag triple + `docs/architecture.md` row-shape/state-key + `docs/customization.md` updated in lockstep. Regression: `tests/test-token-tracking.sh` (35 assertions, CI-pinned, incl. an end-to-end real-transcript hook drive + an off→on backlog-skip case).

**Background-work waiting state (UX).** When the agent dispatched background work (a `run_in_background` agent or Bash task) and yielded the turn to await its completion notification, the turn-end read as a *stop* — the user could not tell "waiting" from "done" — and the stop-guard's "quality checks haven't run" block was misleading when the checks were running in the background. Two-layer fix: **(1) behavioral** — a new output-style "Waiting on background work" response shape + a `core.md` rule require the agent's last line before yielding to be an unmistakable waiting signal (⏳ what's awaited · auto-resumes · no user action). **(2) mechanism backstop (Bash background tasks)** — `posttool-timing.sh` flags a background **Bash** dispatch (`bg_work_dispatched_ts`, set when the `running in background with ID:` marker appears in `.tool_response` — a fork-free pre-filter then a confirming check, so the common PostToolUse path stays fork-free and a foreground command that merely *prints* the phrase does not trip it), and the stop-guard reads+consumes the marker **first, before any early-return path** and appends a conditional "this block is expected — you'll be re-invoked when it completes" note to any block message via `format_gate_block_dual`. The note is **message-only** (the block/release decision is unchanged — *not* a bypass; an umbrella section asserts it cannot suppress a block) and **single-shot** (the stop-guard consumes the marker on every Stop entry, so it cannot stale into a later turn — single-shot consume is the correct expiry because the background-completion notification re-invokes the model as a *synthetic prompt* that does not advance `last_user_prompt_ts`, so a recency gate would never expire; the design was `metis`-pressure-tested and the early-return leak caught by `quality-reviewer`). Other background tools (an Agent dispatch, a CI run) are covered by the behavioral announcement, not this Bash marker. The detection is gated by `time_tracking` (it lives in `posttool-timing.sh`); when `time_tracking=off` the backstop no-ops and the behavioral layer carries the fix. New state key `bg_work_dispatched_ts`. Regression: `tests/test-background-wait-note.sh` (14 assertions, CI-pinned, incl. an early-return leak-path case) + `tests/test-stop-guard-bypass-surface.sh` message-only invariant.

**Manufactured-finish-line fix (objective-contract release requires a fresh audit).** Closes the deepest open-mandate failure, surfaced by a model's own confession: under an exhaustive mandate ("be exhaustive, push to excellence, explore more, no defer") the model silently swaps it for a closeable checklist (the surfaced findings / a wave plan) it CAN finish, completes that subset genuinely, and declares "done" — mistaking the **sample for the ceiling** and **correct for complete**. Every prior gate verified the closed checklist (genuinely satisfied) and could not see the swap; the objective-contract gate released on the drifted model's own `**Objective coverage.**` self-attestation — the corrupt witness. **Fix** (reshaped by `oracle` + `abstraction-critic`, who flagged the initial "add doctrine + a new gate" proposal as solving-by-accretion): the gate's release now requires a **RECORDED fresh-context completeness audit this cycle** (`last_excellence_review_ts > prompt_ts`) *in addition to* the attestation — the model can no longer self-attest past the gate without dispatching an independent fresh auditor. The existing fresh-context auditor (`excellence-reviewer`, axes 1+10) is **sharpened** to ask the anti-narrowing question — "what is the largest worthwhile thing NOT done (the surfaced findings are a SAMPLE, not the ceiling), and for each omission is the reason cost/risk [forbidden] or evidence [tried, measured, made it worse]?" — with *"I can't judge it"* named as avoidance, not evidence, when a bake-and-look check was available. One clause added to `core.md` (the one place doctrine fires — a *judgment to make*, not a bias to suppress): **cost is never a defer reason; relative value reorders the remaining set, never shrinks it; test = "after prioritizing, is the mandate still open?"**. The residual is named honestly as the **4th genuine gap** in `model-robustness.md` (mandate-narrowing / completion-criteria substitution): the fresh audit *lowers* the miss-rate (no sunk-cost in the shipped subset) but does **not** close it — "is the mandate exhausted?" is NLU-hard and a fresh auditor manufactures a better-calibrated finish line too (bounded once, not broken); treating the audit as *proof* of exhaustion is the false-comfort failure. **Cost + honest caveat** (per the /ulw-workflow-change rule): the gate now forces one `excellence-reviewer` dispatch per substantive objective-cycle's first Stop — including long-objective / low-file-count tasks the file-count review gate did not require (excellence-reviewer already auto-fires on complex multi-file work, so the *marginal* cost lands mainly there). And the requirement fully bites only under block-mode (strict autonomy — `no_defer_mode=on` / `quality_policy=zero_steering`, the recommended `/ulw` preset — where the gate holds until the audit+attestation land); in DEFAULT exhaustion mode the 2-block scorecard cap-escape still releases without the audit (named as the second residual in `model-robustness.md` gap #4-ii — this fresh-audit-caught honesty hole was surfaced by dogfooding the new excellence-reviewer on this very change). **Considered and declined** (not deferred-for-cost): a planning-time "forbidden-to-defer set" upstream enrichment — the confessed relabeling is an *execution-time* decision a planning-time partition would not capture, the completion audit already covers it, and a doctrine version would be the accretion both specialists warned against. Regression: `tests/test-objective-contract.sh` (29 assertions, incl. PROBE-3c self-attestation-alone-does-not-clear + an excellence-reviewer-axis content net), F-014f/f2 in `tests/test-stop-guard-bypass-surface.sh` (146; attestation-without-fresh-audit still blocks), T8b in `tests/test-model-robustness-doctrine.sh` (4th gap named + framed not-closed). **Doctrine-edit sign-off:** the `core.md` + `model-robustness.md` edits touch two @-included files with no mechanical net — made under explicit maintainer direction (the user's reflection requesting this fix), per `project_doctrine_over_restriction_assessment.md`.
...
**Workflow tool as opt-in execution substrate + verdict-challenge discipline.** Claude Code shipped a `Workflow` tool (deterministic multi-subagent orchestration — `parallel()`/`pipeline()`, per-agent output schemas, a token budget, background execution + resume). It does **not** obsolete the harness: it is an *execution primitive* (how N agents fan out), orthogonal to oh-my-claude's *behavioral contracts* (whether the agent thinks deeply, verifies, refuses to stop short). Two coordinated changes. **(1) Embrace it as an opt-in substrate** for heavy fan-out (council Phase 8 waves, large audits, migrations), gated on new flag `workflow_substrate` (default `on`; `off` in `minimal`) — this reverses the *default-paradigm* half of the `docs/architecture.md` "graph-of-roles" rejection, whose only-expired premise was "graph state needs a workflow engine we don't have." The lens panel and small/inline work deliberately stay on the lightweight in-thread `Agent` path (the rejection's latency/cost + ULW-momentum reasons still hold — substrate, not paradigm). Touched: `model-robustness.md` Mechanisms 2–3, the `autowork` + `council` SKILLs (council Phase 3 keeps the lens panel inline; Phase 8 gains the `pipeline()` option), `docs/{faq,glossary,architecture,customization}`, and the flag triple-write (`common.sh` parser + `conf.example` + `omc-config.sh` table & all 3 presets) plus a `prompt-intent-router.sh` consumer that injects a suppression directive when `off` — so the flag is *enforced* (soft/model-honored, since the Workflow tool is model-invoked), not merely parsed (a post-implementation `quality-reviewer` caught that `off` was otherwise inert; verdict-challenge applied — the finding reproduced against source, so it shipped). **(2) Verdict-challenge discipline.** A reviewer's / critic's / sub-agent's BLOCK or FINDING is an input to ground in cited evidence (re-read the source; confirm it reproduces) before acting — not a truth to auto-accept, and **not** license to discount findings (default stays *act-on-them*; reflexive dismissal inverts into the v1.42.x F-010 anti-finding bypass). Dogfooding the discipline, a fresh `abstraction-critic` reshaped the initial "add a 9th intellectual-craft instrument" proposal as solving-by-accretion; it ships instead as the **symmetric half of Popper instrument #6** (own-PASS-insufficient ⊕ incoming-verdict-not-automatic) + a `model-robustness.md` Mechanism 8 bullet, wired to the existing `record-finding-list.sh … rejected … 'not reproducible'` mechanism — **instrument count stays eight** (no agent-file lockstep churn). `core.md` gains one clause: *a documented rejection is not permanent truth — when a premise lifts, re-examine* (the very failure that produced the original over-conservative "don't become a workflow engine" stance). **Doctrine-edit sign-off:** the `core.md` / `intellectual-craft.md` / `model-robustness.md` edits touch three @-included files with no mechanical net — made under explicit maintainer direction (the user challenged the over-conservative stance and requested the verdict-challenge build), per `project_doctrine_over_restriction_assessment.md`. Regression: `tests/test-workflow-substrate.sh` (the flag's runtime enforcement — the router injects a suppression directive when `off`) + `tests/test-coordination-rules.sh` Contract 1 (parametric 3-site flag parity — covers `workflow_substrate` automatically), `tests/test-model-robustness-doctrine.sh` + `tests/test-intellectual-craft-doctrine.sh` (verdict-challenge + Workflow-substrate content nets).
...
**Rejected after verification** — three claimed mechanisms did not survive contact with the source:

- `southleft/figma-console-mcp` 6-gauge scorecard — the claim exists only in README marketing; the actual code delegates a11y checks to axe-core and extracts Figma manifests for tokens. No portable rubric to lift.
- `senlindesign/claude2figma` PostToolUse "hook" — literally an `echo "QA: Verify..."` reminder string, not verification logic. No content to port.
- `sickn33/antigravity-awesome-skills/skills/design-taste-frontend/SKILL.md` — substantively a downstream redistribution of `Leonxlnx/taste-skill`. Used the MIT upstream directly instead.

**Port A — XcodeBuildMCP recommendation in all 4 iOS agents.** Added a `## Recommended companion MCP — XcodeBuildMCP` section to `ios-ui-developer.md`, `ios-core-engineer.md`, `ios-deployment-specialist.md`, and `ios-ecosystem-integrator.md`. The MIT-licensed Sentry-maintained MCP server (~5.7k stars) exposes 82 manifest-driven tools grouped by workflow phase — build / test / sim / device / debug / SwiftPM / scaffold / UI-automation — giving the iOS agents a documented executable-surface path instead of round-tripping Xcode operations through Bash. Each block provides install commands (`npx -y xcodebuildmcp@latest mcp` or `npm install -g xcodebuildmcp`), the MCP-config docs path, and the Sentry-telemetry privacy opt-out. Per-agent wording is calibrated to surface: deployment-specialist emphasizes CI/test pipelines, ui-developer emphasizes simulator iteration and accessibility, core-engineer is brief, ecosystem-integrator emphasizes hardware verification of entitlements. Recommendation, not a dependency — the agents operate without it.

**Port B — Vendored `swiftui-pro` bundled skill (model-invoked SwiftUI reviewer).** New `bundle/dot-claude/skills/swiftui-pro/` directory: `SKILL.md` + `LICENSE` + 9 partial-loadable references (`api`, `accessibility`, `data`, `design`, `hygiene`, `navigation`, `performance`, `swift`, `views`). All content lifted verbatim from `twostraws/SwiftUI-Agent-Skill` v1.1 (MIT, © 2026 Paul Hudson — the *Hacking with Swift* author). The skill is model-invoked (no `disable-model-invocation` in frontmatter), so Claude Code auto-loads it when SwiftUI work is in play; partial-load means a prompt like "check for deprecated API" loads only `references/api.md`. The `api.md` reference is the load-bearing lift — it names ~17 SwiftUI rules the model violates from training-data memory: `foregroundStyle()` not `foregroundColor()`, `clipShape(.rect(cornerRadius:))` not `cornerRadius()`, `Tab` not `tabItem()`, `topBarLeading` not `navigationBarLeading`, the `@Entry` macro for `EnvironmentValues` keys, and others — exactly the surface `ios-ui-developer` did not encode itself. Per CLAUDE.md "Adding or removing a skill directory" lockstep: `verify.sh` + `uninstall.sh` + `memory/skills.md` + `README.md` skill table all updated. Regression: `tests/test-swiftui-pro-skill.sh` (33 assertions, CI-pinned). Count drift: 27 → 28 skills, 125 → 126 bash tests.

**Port C — Three new vendored design-craft references.** Added three MIT-licensed files to `bundle/dot-claude/quality-pack/design-craft/`:

- `taste-skill-doctrine.md` (Leonxlnx/taste-skill v2) — anti-slop dials (`DESIGN_VARIANCE`/`MOTION_INTENSITY`/`VISUAL_DENSITY`, each 1-10) plus the em-dash ban. **The em-dash ban is the single most-violated LLM design tell** — zero `—` or `–` permitted anywhere user-visible. Source is ~73KB verbatim, including framework-locked React/Motion/GSAP code skeletons that readers on other stacks can skip.
- `design-for-hackers.md` (ryanthedev/design-for-ai) — 10-category CHECKER + 8-phase APPLIER + Symptom→Chapter lookup + Anti-Rationalization Table. All citations route to David Kadavy's *Design for Hackers*.
- `a11y-doctrine.md` (fecarrico/A11Y.md) — POUR framework under WCAG 2.2 AA / ISO 9241-171 / ADA / EAA, 4-level severity matrix (🔴/🟠/🟡/🔵 with MUST/SHOULD/MAY-FIX semantics), AI Behavior Contract, named anti-patterns (Clickable Divs, Leaked Focus Traps), 5-item Definition-of-Done.

Each file carries a vendoring HTML-comment header naming upstream source, MIT license, vendoring date, and upstream commit SHA. Per CLAUDE.md "Adding or removing a design-craft reference" lockstep, all 4 sites updated: `verify.sh` `required_paths` registers each; the 5 visual-craft consumers (`visual-craft-lens`, `design-reviewer`, `frontend-developer`, `ios-ui-developer`, `frontend-design` SKILL) get a new `## Additional design-craft references (on-demand)` section listing all 3; the UX-trimmed `design-lens` gets a narrower section referencing ONLY `a11y-doctrine.md` (the other two are visual-craft scope — `test-art-taste-doctrine.sh` scope-discipline check still passes at 99/99). Regression: `tests/test-design-craft-additions.sh` (53 assertions, CI-pinned). Count drift: 126 → 127 bash tests.

**Port D — Document VoltAgent `tools:` allowlist convention in CONTRIBUTING.md.** Updated `## Adding Agents` step 3 to describe both permission-boundary conventions: the **allowlist** (`tools: Read, Grep, Glob, ...`) — preferred for new security-sensitive agents because it fails closed when Claude Code adds new tools — and the existing **denylist** (`disallowedTools:`) — looser default, retained for existing agents. The allowlist is the documented Claude Code sub-agent spec and is the convention adopted by [`VoltAgent/awesome-claude-code-subagents`](https://github.com/VoltAgent/awesome-claude-code-subagents) (20.5k stars). Also added a brief "Permission-boundary convention" pointer under "Reviewer-agent additions" with a concrete frontmatter example. Documentation-only — no existing agents migrated. The convention shift is guidance for new agents, not a migration mandate. Verified `model:` per-agent frontmatter is the Claude Code official spec (not a VoltAgent novelty) by reading 5 sample agents from `VoltAgent/awesome-claude-code-subagents/categories/`.

**Port E — Document `ccusage` and `XcodeBuildMCP` as recommended companion tools in README.** Added a `### Recommended companion tools` subsection under `## Power-user setup`:

- [`ryoppippi/ccusage`](https://github.com/ryoppippi/ccusage) (14.7k stars, native Rust binary distributed via npm) — parses Claude Code conversation logs and aggregates token usage and cost against a locked LiteLLM pricing snapshot. The 5-hour `ccusage blocks` aggregation matches Anthropic's actual rate-limit window. Complementary to oh-my-claude's existing rate-limit-headroom surface in the statusline (different concern: token cost, not limit headroom).
- [`getsentry/XcodeBuildMCP`](https://github.com/getsentry/XcodeBuildMCP) — the same MCP recommended inline from the 4 iOS agents in Port A. Centralizing it in the README makes it discoverable to users browsing docs rather than agent files.

Documentation-only — no bash port of ccusage's 5-hour-window logic. The upstream Rust binary is the better install vector; a bash port would duplicate non-trivial work and lose the embedded LiteLLM snapshot.

### Five-port wave from public-Claude-Code-project evaluation (v1.44-pre)

**Deep-evaluated 13 high-star public Claude Code projects** (Citadel, cc10x, disler/hooks-mastery, ECC, thedotmack/claude-mem, russbeye/claude-memory-bank, hudrazine/claude-code-memory-bank, awesome-claude-code, awesome-claude-code-toolkit, musistudio/claude-code-router, ruvnet/ruflo, claude-octopus, Obra/superpowers) and shipped the five ports that closed real gaps in oh-my-claude without softening load-bearing contracts. The other ~50 features evaluated were rejected as duplicates, paradigm conflicts (Python/Node runtime adds), premature (no observed friction), or anti-direction (e.g., meta-agent accelerates agent bloat). Considered-and-declined log in `~/.claude/projects/-Users-xxxcoding-Documents-ai-coding-oh-my-claude/memory/project_v1_44_pre_5port_wave_landed.md`.

**Port 1 — Mechanical circuit breaker on consecutive same-target Bash failures (`circuit-breaker.sh`).** New PostToolUse:Bash hook tracks `target_hash = sha1(tool_name + first_positional_arg + cwd)`. On 3 consecutive same-target failures emits a `CIRCUIT BROKEN — revert + delegate to oracle` directive via `additionalContext`, resets the counter, and sets a 60s quiet window (`circuit_quiet_until`) suppressing re-fires while the model acts on the directive. Failure detection uses the canonical `omc_hook_tool_failed` helper (shared with `record-verification.sh` + `record-delivery-action.sh`), so exit_code / status / success field-shape variance is canonicalized in one place. Background invocations (`run_in_background:true`) are excluded — their tool_response reflects spawn success, not command outcome. Closes the `core.md:128` rule-to-enforcement gap (3-fail rule was model-judgment only). Conf flag: `circuit_breaker=on|off` (default on in maximum/balanced presets; off in minimal). Ported from `SethGammon/Citadel/hooks_src/circuit-breaker.js`. Regression: `tests/test-circuit-breaker.sh` (22 assertions). **Telemetry:** each fire emits a `gate_events.jsonl` row (`kind:"circuit-breaker", event:"fire", target_hash, tool, first_arg`) so the existing `/ulw-report` "Top gates by fire count" aggregation surfaces fire frequency without a dedicated slice. **Known false-positive class:** build-tool dispatchers (`make`, `npm`, `pnpm`, `yarn`, `cargo`, `just`) collapse all sub-commands to a single target hash because the first argv token is shared — three distinct sub-commands failing once each will trip the breaker. Documented in `oh-my-claude.conf.example`; users running dispatcher-heavy workflows should either set `circuit_breaker=off` or accept occasional fires cleared via `/ulw-skip`.

**Port 2 — True stricter-verdict-wins on dimension verdicts.** Three coordinated changes implement cc10x F11 ("for contradictory verdicts across agents, treat the stricter verdict as authoritative; never average or reconcile"): (a) **storage** — `tick_dimensions_with_verdict` and `set_dimension_verdicts` route through `_write_stricter_dim_verdicts_unlocked` (new in `common.sh`), which reads each dimension's current verdict and writes the stricter result on a CLEAN/SHIP (0) < FINDINGS (1) < BLOCK (2) ladder. A CLEAN cannot overwrite a sibling reviewer's FINDINGS on the same dimension. Edit-aware override: when the current verdict's ts predates `last_code_edit_ts`, the new verdict wins outright (fix-and-re-review path clears). `set_dimension_verdicts` now also stamps ts (parity with `tick_dimensions_with_verdict`). (b) **gate** — new stop-guard stricter-verdict-wins gate runs after the review-coverage gate; blocks Stop on any required dim whose verdict is fresh `FINDINGS*` or `BLOCK*`. /ulw-skip bypass works via the top-of-script gate_skip path. (c) **doc** — new "Stricter-verdict-wins invariant" section in `AGENTS.md` documenting both mechanisms. **Pre-Port-2 behavior was last-reviewer-wins** with no gate consumer for `dim_<dim>_verdict` — a sibling CLEAN running after a FINDINGS silently dropped the findings. `show-status.sh` display adapted to lead with the "findings reported" frame whenever verdict is FINDINGS/BLOCK (previously relied on the ts-empty branch which is no longer reachable). Ported from `romiluz13/cc10x/plugins/cc10x/skills/cc10x-router/SKILL.md` F11. Regression: `tests/test-stricter-verdict-wins.sh` (26 assertions).

**Port 3 — Zero-finding evidence anchor rule in `quality-reviewer.md`.** New priority 12 in the agent prompt: if no defects are found, cite ≥2 concrete file:line evidence anchors in the **Summary** section proving the reviewer read the diff. If 2 anchors cannot be produced, downgrade to `VERDICT: FINDINGS (1)` with a meta-finding `{severity:"low", category:"other"}` requesting deeper read. Defends against the rubber-stamp-CLEAN failure mode the historical `unknown ×247` cluster surfaces. Prompt-side rule only, no mechanical gate. **Effectiveness signal:** the FINDINGS(1) downgrade emits a `meta-finding` with `category=other, severity=low`, claim substring "Reviewer unable to produce ≥2 file:line evidence anchors" — that row lands in `findings.json` (and `/ulw-report`'s existing findings-by-category aggregation surfaces it). Expect ≥1/50 reviews if the rule fires; zero across weeks of `/ulw-report` data means either the prompt's not landing OR rubber-stamp incidents aren't happening (the second interpretation is the win condition). Ported from `romiluz13/cc10x/plugins/cc10x/agents/code-reviewer.md` zero-finding gate. Regression: `tests/test-quality-reviewer-zero-finding-rule.sh` (7 assertions).

**Port 4 — Anti-anchoring rule for `metis` under plan-complexity nudge.** New section in `metis.md`: when dispatched as a follow-up to a high-complexity `quality-planner` plan, metis treats every planner-stated invariant ("we already have X", "the contract is Y", "the implicit invariant is Z") as a **claim to verify against current code**, not a premise to accept. This is verification-not-restriction — metis SHOULD read the planner's WHY when WHAT names a specific behavior, but verifies the claim independently. `reflect-after-agent.sh` PLAN COMPLEXITY NOTICE updated to surface the directive so the main thread tells metis to apply the rule. The rule was dogfooded in this wave: the initial draft used a too-restrictive "Read only: (a)(b)(c)" form that would have blocked metis from finding F-1 (the storage-asymmetry bug Port 2 fixes); the dispatch surfaced the over-restriction and the rule was reframed before shipping. Ported from `romiluz13/cc10x/plugins/cc10x/agents/plan-gap-reviewer.md` "forbidden to load planner rationale" framing. Regression: `tests/test-metis-on-plan-gate.sh` T13 (extended with 2 anti-anchoring assertions).

**Port 5 — Transcript-archive Stop hook (`stop-transcript-archive.sh`, opt-in).** On every clean Stop, if `transcript_archive=on`, copies Claude Code's session JSONL (from the hook input's `transcript_path`) and normalizes it to a JSON array at `~/.claude/quality-pack/state/<project_key>/<session_id>/transcript.json`. Idempotent — existing destination is left untouched. `jq -s '.'` slurp preferred; falls back to `cp` (with `.jsonl` suffix) when jq unavailable. **Skips on fatal matchers** (`rate_limit` / `authentication_failed` / `billing_error` / `max_output_tokens`) — `stop-failure-handler.sh` owns the fatal-stop window and the JSONL is mid-flush. **Honors `is_stop_failure_capture_enabled` for privacy parity** — the transcript carries the same secrecy surface as `resume_request.json`, so the same opt-out applies. Default OFF (disk cost 50-500 KB/session). Conf flag: `transcript_archive=off|on`. **Defensive fatal-stop guard.** The matcher case statement is wired to Stop (not StopFailure); per `docs/architecture.md` `matcher` is currently a StopFailure-only field, so the guard is **dead code in production today** — it's defensive future-proofing against a Claude Code hook-surface change that would forward a fatal `matcher` on Stop. If a maintainer ever observes the case matching in production, that signals the hook surface changed and `stop-failure-handler.sh`'s fatal-window logic should be re-audited. Ported from `disler/claude-code-hooks-mastery/.claude/hooks/stop.py` transcript extraction. Regression: `tests/test-stop-transcript-archive.sh` (12 assertions).

**Verification.** New: `tests/test-{circuit-breaker, stop-transcript-archive, stricter-verdict-wins, quality-reviewer-zero-finding-rule}.sh` (67 new assertions total). Extended: `tests/test-metis-on-plan-gate.sh` T13 (+2 assertions, now 29 total). Cross-cutting: `tests/test-coordination-rules.sh` clean (154/0; 4 new tests CI-pinned in `.github/workflows/validate.yml`); `tests/test-e2e-hook-sequence.sh` clean (399/0); `tests/test-common-utilities.sh` clean (536/0); `tests/test-agent-verdict-contract.sh` clean (444/0); shellcheck `--severity=warning` clean across `bundle/`; `python3 -m json.tool` clean across all JSON. Two metis findings (F-1 dim-helper asymmetry, F-2 canonical-failure-detector reuse) became Port-2/Port-1 implementation changes; F-5 (privacy parity + fatal-stop skip), F-6 (quiet window), F-7 (background skip) shipped per the metis recommendation. The plan-complexity nudge + Port-4 anti-anchoring rule was dogfooded in this wave — the rule's first dispatch caught F-1, validating the rule shape before the rule shipped.

**Coordination Rules touched.** Conf-flag triple (parser + emit_known_flags + conf.example) for `circuit_breaker` and `transcript_archive`; preset updates in `omc-config.sh` for all three presets; new entries in `verify.sh` required_paths + hook_scripts + required_hooks; new `settings.patch.json` entries (PostToolUse:Bash for circuit-breaker, Stop for transcript-archive); count drift fix in `README.md`, `AGENTS.md`, `CLAUDE.md` (35→37 autowork scripts; 121→125 bash tests).

**Post-ship review findings addressed (commit follow-up to `d77abea`).** quality-reviewer + excellence-reviewer dispatched on the wave produced 7 LOW + 1 MEDIUM findings (zero defects in actual logic — all docs/wording/measurement-signal). All addressed in a follow-up commit before the wave is considered complete: (1) `stop-transcript-archive.sh` docstring reworded to name the fatal-matcher case as defensive future-proofing rather than active interaction; (2) `oh-my-claude.conf.example` circuit_breaker entry now names the build-tool first-arg collision class (`make test`/`make build`/`make lint` all hash same target); (3) `common.sh:3913` comment fixed to reference the actual gate name "stricter-verdict-wins gate" rather than the nonexistent "findings-pending check"; (4) memory file `project_v1_44_pre_5port_wave_landed.md` re-anchored on entry-count threshold instead of a stale snapshot, and the PreCompact-rejection cite now includes the path (`bundle/dot-claude/quality-pack/scripts/`); (5) Port 3 effectiveness-signal added to CHANGELOG (FINDINGS(1) downgrade with `meta-finding category=other` is the measurable falsifier in `/ulw-report` findings-by-category); (6) variant-coverage rejection rationale strengthened in the memory file — the prior "disposition substitutes for mechanism" rationale was wrong (per `model-robustness.md` Mechanism 1, mechanism IS stronger than disposition when the model can rationalize); the actual reason is scope, and variant-coverage is named as the FIRST candidate for the next port wave.

**Excellence-reviewer also independently confirmed the dogfood validation is genuine.** The metis F-3 finding (`discovered_scope.jsonl:6efd188b5130`) explicitly names the over-restrictive "(a)(b)(c)" draft as a contradiction with metis's existing item 7, and its `recommended_fix` text matches the committed `metis.md:24-32` rule body almost verbatim. The dogfood dispatch produced both the storage-asymmetry F-1 finding (which became the Port 2 patch) AND the rule-flaw F-3 finding (which reshaped the rule's wording) — structured receipts predate the rewrite.

### Release-attestation pipeline + supply-chain hardening

**Closes the supply-chain integrity gap on the public install path.** Before this change, the `curl … | bash` bootstrapper was integrity-anchored to TLS-to-GitHub alone; nothing bound the bytes a user executed to the bytes that came out of a CI build of a tagged source tree. This release adds GitHub artifact attestations for every published release asset (Sigstore-signed via OIDC) plus a canonical-rebuild gate that fails closed when the published tarball drifts from the source tree at the release commit.

**Pipeline.** New `.github/workflows/attest-release-assets.yml` fires on `release: published` and `workflow_dispatch`. The workflow (a) rebuilds the canonical release assets from `refs/tags/vX.Y.Z^{commit}` via `tools/build-release-assets.sh`, (b) downloads the published assets via `gh release download`, (c) `cmp -s`-verifies that the published bytes match the canonical rebuild, then (d) issues a Sigstore attestation via `actions/attest`. The workflow holds the minimum permission set for OIDC signing: `contents: read`, `id-token: write`, `attestations: write` — no pull-request, packages, or actions-write surface. `workflow_dispatch` accepts a `tag` input that is regex-validated `^v[0-9]+\.[0-9]+\.[0-9]+$` before any shell expansion, eliminating workflow-injection through tag names. Both `actions/checkout` and `actions/attest` are SHA-pinned to 40-char commit refs with `# v4.x.y` trailing annotations so the floating-tag surface (the exact compromise vector for tj-actions/changed-files in March 2025) cannot reach the id-token-write workflow.

**Deterministic rebuild.** `tools/build-release-assets.sh` produces byte-deterministic `oh-my-claude-vX.Y.Z.{tar.gz,zip,SHA256SUMS}` from `git archive`. The Python heredoc fixes mtimes to `315532800` (1980-01-01), uids/gids/uname/gname to zero, mode bits to source-tree literals, ZIP timestamps to `(1980, 1, 1, 0, 0, 0)`, and overwrites the gzip header OS byte at offset 9 with `0x03` (Unix) so cross-platform rebuilders cannot drift on the platform-default OS field. Verified locally: two back-to-back builds of `v1.43.0` produce byte-identical archives (tar.gz + zip + SHA256SUMS).

**Verifier matrix.** `tools/verify-published-release.sh` is the user-facing single-tag verifier; it composes `verify-published-release-{title,body,state,assets,attestations}.sh` (one tool per concern). `tools/audit-published-releases.sh` batch-runs the verifier across `gh release list`. `tools/wait-for-release-attestations.sh` polls for the workflow run associated with a release tag and falls through to `workflow_dispatch` when none is registered. `--watch-timeout` (default 1800s, requires `timeout(1)`/`gtimeout` on PATH) bounds the `gh run watch` call so a hung Actions run cannot block the caller indefinitely. The initial poll skips terminal-failure runs (status=completed with conclusion in {failure, cancelled, timed_out, action_required, startup_failure, stale, neutral, skipped}) via best-effort `gh api repos/.../actions/runs/{id}` lookups — falls through to "treat as viable" when the lookup is unavailable so tests using the gh stub do not require stub extension. Per-tick progress logs (poll N/M, dispatch-poll N/M) replace the silent 60s polling window so a maintainer running this in a release-day sequence can see what the script is doing.

**`cmp -s` mismatch diagnostic.** The workflow's "Verify published assets match canonical rebuild" step now walks all three assets in one pass, emits a Markdown table to `GITHUB_STEP_SUMMARY` naming each asset's expected/actual SHA-256, expected/actual size, and first-differing byte offset from `cmp` (no `-s`), then exits non-zero. Pre-fix, the step aborted on the first `cmp -s` failure with no diagnostic — a maintainer reading the failed run had to rebuild locally to learn which asset diverged.

**Release-readiness composer family.** Four umbrella audits, each composing the per-concern verifiers for a distinct audience: `tools/verify-install-readiness.sh` (first-time installer's perspective), `tools/verify-professional-readiness.sh` (cross-domain product proof), `tools/verify-distribution-readiness.sh` (release-distribution audit), and `tools/verify-project-readiness.sh` (one-shot maintainer release-candidate audit). `tools/install-state-report.sh` is the AI-installer probe documented in `AGENTS.md` for the Agent Install Protocol — emits a `--json` report whose top-level keys are pinned by `tests/test-install-state-report.sh` (36 contract assertions); each `--*-summary` mode is order-dependent per the source-level dispatch chain.

**install-remote.sh trust-model banner.** The `curl … | bash` bootstrapper now prints an explicit trust-model banner naming the actual guarantees: integrity on this path = TLS to GitHub; the published-release attestations protect MAINTAINERS verifying releases via `tools/verify-published-release-attestations.sh`, not END USERS running the one-liner (the bootstrapper `git clone`s rather than downloading the attested tarball). Suppressed by `OMC_TRUST_BANNER_SUPPRESS=1` for CI. `OMC_EXPECTED_SHA` minimum length is bumped from 7 to 12 with a soft deprecation band (7-11 chars emit a yellow warning but still proceed) — full 40-char SHAs from the release page are the recommended posture. The 7-char floor was vulnerable to documented natural collisions on git trees of any meaningful size.

**install.sh trap lifecycle.** The EXIT trap is now registered BEFORE `acquire_install_lock`. Pre-fix, if `acquire_install_lock` failed mid-mkdir (exhausted attempts, race against another install), the trap never registered and the partial `INSTALL_LOCK_DIR` leaked. `release_install_lock` is now pid-scoped — it only removes locks whose pidfile matches `$$`, so the new pre-acquire trap cannot erase a contending process's lock when our own acquire fails.

**Verification.** All 657 assertions in `tests/test-release.sh` pass after the hardening (was 655 — two assertions updated: T31 now matches a SHA-pinned `actions/attest@<40-hex> # v4.x.y` form, T57 splits the wait-path concatenation assertion into two independent substring checks now that per-tick progress lines precede the "watching workflow run" announcement). New nets: `tests/test-install-state-report.sh` (36 assertions), `tests/test-realwork-eval-suite.sh` T22 (by-design-missing-fixture contract — 1 assertion across 20 scenarios). Bash 57 install-remote tests, 38 professional-readiness, 24 install-readiness, 30 project-readiness, all clean.

**Stub-format drift surface closed.** `mk_stub` / `mk_json_stub` extracted to `tests/lib/composition-stubs.sh`. The previous arrangement (four hand-rolled emitters across `test-{project,professional,install}-readiness.sh` and `test-realwork-eval-suite.sh` with minor variations) was the readiness-composer test family's equivalent of the value-shape drift surface `tests/lib/value-shapes.sh` closes. The lib documents the `TMP_DIR`-set-by-caller contract.

### Outcome-oriented realwork eval scenarios — advisory, quantitative, regulated, document, presentation

13 new scenarios in `evals/realwork/scenarios/`: `advisory-{code,data,legal,operations,research,writing}-guidance.json`, `quantitative-{kpi-brief,workbook-model}.json`, `regulated-compliance-memo.json`, `document-policy-docx.json`, `presentation-board-deck.json`, `mixed-{cutover-checklist,rollout-migration}.json`, `operations-plan.json`, `research-brief.json`, `scholarly-review.json`, `writing-proposal.json`. Each declares the canonical schema (id, risk, fixture path, prompt, required_outcomes, budgets) and is exercised by `tests/test-realwork-eval-suite.sh` scoring assertions. The fixture paths intentionally do NOT resolve on disk — the README documents that realistic fixtures are project-specific and user-provided. T22 pins this by-design contract so a future contributor cannot file the "missing fixtures dir" bug or commit synthetic fixtures (the inverse-of-out-of-scope failure mode).

### Classifier domain-inference refactor: per-signal scorer helpers

`bundle/dot-claude/skills/autowork/scripts/lib/classifier.sh` decomposes `infer_domain` into five named scorer helpers — `_coding_signal_score`, `_operations_signal_score`, `_quantitative_signal_score`, `_regulated_signal_score`, `_writing_signal_score` — each with a paired `prompt_has_<X>_signal` boolean wrapper. `infer_native_artifact_kind` is a new classifier that returns `spreadsheet | presentation | document | none` based on noun-shape detection in the prompt; downstream consumers can route to the right deliverable specialist without re-matching the regexes. The decomposition preserves every existing classifier outcome (verified by `tests/test-intent-classification.sh` and `tests/test-specialist-routing.sh`) and is a precondition for the new realwork eval scenarios above.

### Context-cost trim: core.md compressed 56→16 KB (71.9% reduction, ≈9–10k preamble tokens recovered)

**No binding contract changed; this is pure compression.** Every rule, every FORBIDDEN entry, every operational pause case, every contract-test invariant is preserved verbatim — only the prose around the rules was tightened.

**Closes the Claude Code "Large core.md will impact performance (55.8k chars > 40.0k)" warning the user surfaced directly (2026-05-25).** Before this change, `core.md` was 56,066 chars — large enough to trip the editor's performance warning, and large enough that the always-loaded preamble (the @-include chain in `bundle/dot-claude/CLAUDE.md`) carried ≈14k tokens before any user prompt ran. The yesterday's `7077670 context-cost belt trim: −9.7 KB across 5 always-loaded doctrine files` shaved 2.1 KB off `core.md` as part of a wider cross-file trim; this is a focused second pass on `core.md` alone.

`core.md` reduced from 56,066 → 15,744 chars (**71.9% reduction, 39.4 KB / ≈9–10k tokens recovered from the preamble**). The saving is a *one-time per session* recovery from the always-loaded `@`-include chain (paid once when the session opens, amortized across every prompt via Anthropic's prompt cache) — it is NOT a per-prompt saving. Per-section reduction: Who `/ulw` is built for 79%, Why `/ulw` exists 77%, Thinking Quality 63%, Workflow 73% (the largest absolute saving, ~15 KB), Code & Deliverable Quality 76%, no-defer contract 73%, No-Out-of-Scope contract 68%, Anti-Patterns 72%, Failure Recovery 24%. The user's framing was *"trim it aggressively as needed, the shorter the better, don't lose its meaning, aim for 90%"*; 72% is the principled floor at which the compression preserves every binding contract — pushing further would have removed FORBIDDEN entries that the contract sections themselves enumerate as the binding rejection list (which the file's own anti-pattern audit forbids).

**Preserved verbatim** (all 62 contract-test assertions still pass): every literal phrase the regression nets grep for — `do NOT optimize this away` (both contract headers), `Soft-warn instead of hard-block`, `Collapsing the dual-failure-mode framing`, `Softening the v1.40.0 no-defer contract`, `## The v1.44 No-Out-of-Scope contract (load-bearing — do NOT optimize this away)`, `FORBIDDEN: Softening the v1.44 No-Out-of-Scope contract`, `dispatch a fresh-context sub-agent, do NOT announce a session boundary`, `Two failure modes are equally weighted`, `governs deferral of REMAINING WORK`, `load-bearing rule, not a soft suggestion`, `Engage at full cognitive depth on every prompt`, `Favor verification over abstraction`, `Deliberation comes first, action comes second` (and its positional invariant — must appear BEFORE `In maximum-autonomy mode, default to action` per `tests/test-depth-prime-contract.sh` T13). Plus the structural enumerations: all 5 operational pause cases, all 3 no-defer options, all 3 Serendipity conditions, all 7 stop-guard bypass surfaces, all 8 FORBIDDEN-softening entries in the v1.40.0 contract, all 4 FORBIDDEN-softening entries in the v1.44 contract, and all 13 entries in the top-level Anti-Patterns list.

**Compressed**: the prose around every rule — rationales, examples, real-incident anecdotes that anchored the rule's origin, cross-references to prior memory files, version-history asides. Verbose recovery-shape sub-bullets in Workflow ("Too heavy for this session" rationalization) collapsed to shape→action one-liners while preserving the recognition map for all 6 named shapes (multi-hour, long-context drift, UI-render verification, "candidates for next session," triage-as-stop, permission-coded stop). The specialist-agent enumeration in Workflow compressed to a name-list + pointer to `skills.md` (where the full decision tree already lives and which is loaded via the same @-include chain). Hygiene rule kept its bash 3.2 trap-syntax cite and the concrete artifact list (the parenthetical is what makes the abstract term "transient artifact" actionable; removing it would have weakened a rule that closed real telemetry-observed orphan damage). The v1.40.0 contract's `Why` paragraph (canonical-user reasoning) preserved verbatim — it is the contract's anchor. The v1.44 contract's user-quote (2026-05-23 *"Everything is within scope. There is no such things as out of scope […]"*) preserved — same reason.

**editor-critic loop ran before commit** (per the post-edit reviewer-chain contract under the no-defer rule). 4 force-loss findings + 1 structural-completeness finding, all addressed inline:

- **F-1 acted (verbatim restore):** pause-list lost the "exhaustive" marker — future agent could plausibly add a sixth case. Restored: `Five **exhaustive** pause cases (operational, not judgmental) — anything outside this list is the agent's call, not a sixth case`.
- **F-2 acted (symmetric restore):** the v1.44 contract's FORBIDDEN entry for *"Disable `god_scope_on_bare_prompt` by default"* lost the MUST/non-negotiable framing that the sibling `no_defer_mode=on` entry kept. Restored: `Recommended preset MUST ship 'on' — non-negotiable, same reason as no_defer_mode=on`.
- **F-3 declined with WHY (rejected as not-a-defect):** finding asked to restore the per-specialist trigger list to Workflow (~2 KB of routing rules). Declined because `skills.md` is also @-included via `bundle/dot-claude/CLAUDE.md:4` — both files load together or neither does. The compressed `full decision tree in skills.md` pointer is sufficient; restoring would duplicate the routing rules across two memory files (the exact lockstep-drift surface Contract 6 exists to prevent).
- **F-4 acted (verbatim restore):** Serendipity Rule lost the meta-application rule *"If you find yourself arguing whether a condition holds, the answer is defer, document, and surface."* Restored — this is the rationalization-counter that distinguishes the rule from "fix anything that's wrong near where you're working."
- **F-5 acted (structural completeness):** `Why /ulw exists` section was so deflated that a first-time reader could not see the dual contracts are equal-weight. Added: *"These are equal-weight — neither contract can be relaxed independently."*

**Stale line-anchor fix.** `docs/enforcement-classes.md:25` referenced `core.md:95` (the pre-trim location of the Hygiene rule). De-pinned to `core.md` — line-number anchors into prose-doctrine files rot on every edit; the rule is greppable by its `Hygiene: clean up after yourself` heading.

**Verification.** 733+ assertions pass across the regression nets: `test-no-defer-contract.sh` (21), `test-no-out-of-scope-contract.sh` (27), `test-depth-prime-contract.sh` (14), `test-coordination-rules.sh` (142 — Contract 6 memory-file lockstep), `test-stop-guard-bypass-surface.sh` (130), `test-e2e-hook-sequence.sh` (399), plus shellcheck `--severity=warning` and JSON validation both clean. No new flags added/removed (no lockstep impact). `bundle/dot-claude/quality-pack/memory/core.md` and the installed `~/.claude/quality-pack/memory/core.md` synced byte-for-byte.

**Why not 90%.** The remaining 15.5 KB is bound by load-bearing structure the file's own anti-pattern audit forbids removing. Cutting further would have started removing FORBIDDEN entries that the contract sections themselves enumerate as the binding rejection list — which the file explicitly labels as load-bearing under "FORBIDDEN — softening the contract." Going below 10-11 KB would require accepting named tradeoffs (dropping the v1.44 user-quote anchor, compressing Hygiene to lose the bash 3.2 trap syntax, reducing FORBIDDEN entries to trigger-phrase only without rationale) — each a real loss of meaning the user can opt into on a future pass if the per-session token recovery is worth more than the auditability of *why* each soften-proposal is rejected.

### Intellectual-craft doctrine — operational moves of mind from named physicists and philosophers

**Closes the "workflow lacks the ability to properly think" failure mode the user named directly (2026-05-24).** Before this change, the cognitive-quality memory chain (`core.md` Thinking Quality, `model-robustness.md` mechanism map) contracted for *depth on every prompt* and named the *structural mechanisms* that buy quality independent of model effort — but did not name the **operational habits of mind** that researchers and philosophers developed across centuries to keep themselves from fooling themselves. The user's framing (*"approach to tasks lacks refinement, not just logic but a properly mindset"*) named this gap: the doctrine had vocabulary for contracts and mechanisms, but not for the moves of mind a Cambridge-trained researcher would expect from refined work.

`bundle/dot-claude/quality-pack/memory/intellectual-craft.md` is a new globally-loaded memory file (~3000 words) distilling eight operational moves from named figures: **#1 the Feynman test** (don't fool yourself — name the negation of your claim); **#2 the Wheeler test** (every great problem you can state simply — write the question, not the prompt); **#3 the Socratic move** (interrogate the framing before adopting it); **#4 the Fermi probe** (order-of-magnitude before precision); **#5 the Bohr probe** (push to the limits — N=1, N=∞, concurrent, offline); **#6 the Popper test** (name the falsifier before declaring verified); **#7 the Wittgenstein discipline** (don't fake precision — could a careful reader misread your certainty?); **#8 the Lakatos move** (distinguish hard core from protective belt — which layer does this proposal touch?). Each instrument is framed as a *concrete move the agent makes*, not an exhortation to think harder, and is paired with the *specific anti-pattern it catches in our harness* (pattern-match-without-understanding, verification theater, confidence miscalibration, edge-case blindness, contract softening, etc.).

The doctrine opens with a defensive `What this file is — and is NOT` section that explicitly closes the doctrine-layering regress `model-robustness.md` warned against: this is NOT a new instruction layer, NOT a substitute for the structural mechanisms, NOT adversarial framing, and NOT consolidate-able into `core.md` Thinking Quality. The same anti-pattern audit that softening the no-defer contract requires applies to removing or merging this file. The dual-failure-mode contract grows from `depth + no-defer` to `depth + no-defer + intellectual-craft`, all load-bearing.

Wired into the global @-include memory chain in `bundle/dot-claude/CLAUDE.md` (between `model-robustness.md` and `compact.md`) so the doctrine loads on every session — distinct from `art-taste-doctrine.md` which is scoped to design surfaces and loaded on demand. The justification: thinking happens on every session, not just design sessions; the ~3000-word context cost is paid once per session and bought in exchange for naming the moves of mind the user invokes `/ulw` to receive.

Inline references threaded into 7 deliberative specialist agents, each with an `Intellectual-Craft Calibration` section that anchors the canonical doctrine path AND inlines the 2 instruments most load-bearing for that lens: **`oracle`** (Feynman test + Popper test — root-cause integrity); **`metis`** (Fermi probe + Bohr probe — plan limits); **`abstraction-critic`** (Socratic move + Lakatos move — paradigm fit); **`excellence-reviewer`** (Feynman test + Wittgenstein discipline — fresh-eyes honesty); **`quality-planner`** (Wheeler test + Fermi probe — decision-completeness); **`prometheus`** (Wheeler test + Socratic move — interview-first scoping); **`divergent-framer`** (Socratic move + Lakatos move — option-space honesty). Agents start with their own system prompts and do not inherit the user's @-include chain the same way the main thread does — inline references are how the doctrine load-bears within the specialists where deliberation actually happens.

4-site coordination lockstep (per the project `CLAUDE.md` memory-file rule): (1) the doctrine file at `bundle/dot-claude/quality-pack/memory/intellectual-craft.md`; (2) `@~/.claude/quality-pack/memory/intellectual-craft.md` in the bundled `CLAUDE.md` @-include list; (3) `${CLAUDE_HOME}/quality-pack/memory/intellectual-craft.md` in `verify.sh` required_paths; (4) `tests/test-coordination-rules.sh` Contract 6 auto-discovers any new `*.md` in the memory dir — no edit needed there. Plus the cross-cutting surfaces: CI pin in `.github/workflows/validate.yml`, test count bump `114→115` in `README.md` + `AGENTS.md`. The new dedicated regression net `tests/test-intellectual-craft-doctrine.sh` (52 assertions, all passing) asserts: doctrine file integrity (each of the 8 figures named, each of the 8 instrument headings present, the `What this file is — and is NOT` defensive section, the `Coupling to the rest of the doctrine` section, ≥8 `The move:` markers, ≥8 `Catches:` markers); @-include wiring; verify.sh wiring; the seven specialist agents each reference the canonical doctrine path AND anchor their 2 assigned instruments AND carry the `## Intellectual-Craft Calibration` heading anchor (closes a drift surface where a future edit could move instrument names out of the calibration section); and the declined-instruments audit log section is present (so future sessions see what was weighed before re-proposing it).

Three reviewer passes adjusted the deliverable inline before commit (post-draft refinement, all addressed under the no-defer contract): **abstraction-critic** caught a Lakatos category error in the file's own `What this file is — and is NOT` section — the original wording treated the *whole file including the specific 8-instrument lineup* as hard core, when by the file's own #8 Lakatos rule the doctrine's *existence* is hard core but the *specific instruments* are protective belt; the section was rewritten to make the distinction operationally answerable. **quality-reviewer** caught the Wheeler-quote attribution as a paraphrase-presented-as-direct-quote and a redundant "at the doctrine level" qualifier in the consolidation clause; both softened to match the Primary Sources hedge and the Lakatos paragraph's already-explicit belt carve-out. **excellence-reviewer** named four completeness gaps: missing Cambridge tradition acknowledgment (Wittgenstein/Russell/Hardy/Ramsey sit in the analytic-philosophy spine underneath the doctrine), missing instrument for "what to think about" (Hamming taste-of-importance), prometheus's calibration overlap with divergent-framer on the Socratic move, and the executor-agent exclusion. Resolutions: (a) Cambridge note added to Primary sources naming the analytic lineage and the Hardy/Ramsey decline; (b) Hamming explicitly declined in a new `Considered and declined` log section with reasoning — the move's natural granularity is cross-session research-life selection, and the within-prompt application is already implemented as the v1.44 god-scope-scan router directive; (c) prometheus's second calibration swapped from Socratic move → Fermi probe (test assignment updated); (d) executor-agent threading explicitly declined in the same log section — domain executors do execution downstream of deliberation and inherit the doctrine through the main thread that dispatches them; the criterion for inline threading is "deliverable IS a piece of reasoning." The declined-log itself is an instance of #7 Wittgenstein discipline (do not fake the precision of "I covered everything"; name what you did not).

### Art-taste doctrine — canonical art-historical grounding for design-side agents

**Closes the "workflow has no art taste" failure mode the user named directly (2026-05-24).** Before this change, the design-side agents (`visual-craft-lens`, `design-reviewer`, `frontend-developer`, `frontend-design` SKILL, `ios-ui-developer`, `design-lens`) critiqued and produced UI in generic vocabulary — "palette," "hierarchy," "spacing" — without grounding in canonical art-historical principles. Generic vocabulary produced generic taste; the workflow had no eyes for what makes a Rothko *a Rothko* vs a colorblock, or a Vermeer interior vs a stock photo of an interior.

`bundle/dot-claude/quality-pack/design-craft/art-taste-doctrine.md` is a new load-bearing reference (~4000 words after the §11 motion/digital expansion) distilled from primary sources: Rams's *Ten Principles*, Albers's *Interaction of Color*, Tschichold's *New Typography*, the Vignelli *Canon*, Müller-Brockmann's *Grid Systems*, Klein's IKB monochrome, Hokusai's three-pigment discipline, Vermeer's optical technique, Cartier-Bresson's decisive moment, Fukasawa's "Without Thought," plus Mondrian, Caravaggio, Klimt, Bauhaus, Memphis, the 12 Disney animation principles, Maeda's *Laws of Simplicity*, Tufte's data-ink discipline, Susan Kare's pixel-scale icon work, Don Norman's affordances, and Bret Victor's direct-manipulation arguments. Ten sections — color masters, composition masters, restraint-vs-maximalism, movement principles, typography, industrial design, photography, §8 non-obvious calls (the highest-leverage section), §9 how-to-use, §10 named anti-patterns, §11 motion + digital natives.

The 6 consuming surfaces (5 agents + 1 skill) each got an Art-Taste Calibration section: (a) the canonical doctrine path as a one-tool-call deep reference, (b) inlined high-leverage principles so baseline taste applies even without the deep Read, (c) a §10 anti-pattern catalog reference, and (d) a closing diagnostic asking *"which canonical principle does this honor?"* — if the answer is "none," the agent has a default, not a decision. `design-lens` receives a UX-trimmed 3-principle variant (Cartier-Bresson decisive-moments + Fukasawa "Without Thought" + §8 person-vs-committee) scoped to its UX lane; the other 5 receive the visual-craft 8-principle variant. `ios-ui-developer` adapts the Rothko/Liquid Glass framing platform-aware (iOS 26+ Liquid Glass, iOS 17–25 Materials, earlier translucent overlays) so the principle remains portable across deployment targets.

4-site coordination lockstep: (1) doctrine file, (2) `verify.sh` `required_paths`, (3) inline reference + Art-Taste Calibration section in each of 6 consuming surfaces (5 agents + 1 skill), (4) `tests/test-art-taste-doctrine.sh` regression net (asserts file existence, 16 canonical citations, 15 section anchors including the visual section headings and bold inline entries, §8/§10 anchors, verify wiring, each of 6 agents references doctrine path + inlines baseline anchors, CLAUDE.md mentions the surface and Coordination Rule). `Müller-Brockmann` deliberately checked via the ASCII-safe `Brockmann` substring (the `ü` is encoded NFC `c3 bc`; a macOS HFS+/APFS NFD roundtrip would silently fail byte-literal `grep -F`). `CLAUDE.md` Coordination Rules grew a new entry naming the design-craft 4-site lockstep alongside the existing memory-file and skill-directory lockstep entries. `AGENTS.md` documents `design-craft/` as the workflow's 6th architectural surface (alongside memory/, agents/, skills/, scripts/, output-styles/). CI job added to `.github/workflows/validate.yml`. README + AGENTS test counts bumped 113→114.

The doctrine is intentionally **NOT** added to the global `@`-include chain in `bundle/dot-claude/CLAUDE.md` — it's an on-demand reference loaded by the 6 design agents, not a ~4000-word tax on every non-UI session. Same shape as `bundle/dot-claude/oh-my-claude.conf.example` — installed and authoritative, but referenced rather than auto-loaded.
...
A parallel SRE / security / data / quality-reviewer / excellence-reviewer / abstraction-critic / planner council audited the in-flight diff for release readiness and surfaced 7 verified follow-up gaps — all closed inline before commit per the no-defer contract.

**Headline change (mandate → opt-in)**

- **Shipped form.** New conf flag `agent_first_gate=on|off` (env: `OMC_AGENT_FIRST_GATE`), **default `off`**. Wraps both enforcement surfaces: (1) the PreTool BLOCK in `pretool-intent-guard.sh` (the first-mutation deny) and (2) the Stop-time backstop in `stop-guard.sh` (the after-the-fact agent-first block). Telemetry (`first_mutation_ts`, `agent_first_gate_blocks`, `first_mutation_tool`, `agent_first_gate_state`) keeps capturing unconditionally — the change is mandate-vs-tool, not visibility — so `/ulw-report` can compare opt-in vs opt-out outcomes per-row.
- **Framing — this is removal of a uniform tax, NOT softening of a contract.** The no-defer contract (load-bearing per `core.md` "v1.40.0 no-defer contract") is unaffected. Per the `abstraction-critic` paradigm critique: the mandate was the right shape when (a) specialists ran on a smarter tier than main, (b) the depth-on-every-prompt rule did not exist yet, and (c) reflexive-dispatch round-trip cost less than drift-failure cost — none of those three premises hold now. The `core.md` "FORBIDDEN — softening the contract" list explicitly excludes mandate-to-tool conversions from its anti-pattern class.
- **What carries the load now.** Existing mechanisms: `core.md` "Thinking Quality" (depth-on-every-prompt, load-bearing) + `model-robustness.md` Mechanism 2 (sub-dispatch as TOOL not mandate). The `Agent` tool is always there; under `off` it's the tool the model reaches for when warranted, instead of a precondition fired on every prompt.
- **Opt-in remains available.** Set `agent_first_gate=on` to restore the legacy mandate. Recommended when (a) training a new workflow habit, (b) running with `model_tier=economy` (Sonnet specialists vs Sonnet main — the smartness-gap assumption still holds), or (c) the active session is drift-prone on a single surface.
- **Serendipity catch.** `_bash_command_may_mutate_workspace` was false-positive-matching literal `<unset>` / `<TODO>` inside double-quoted Bash args as shell redirects. Fix: strip `"..."` and `'...'` content before the redirect regex (narrowed to the redirect branch only; verb regexes still see un-stripped input). Pre-existing `bash -c "<destructive verb>"` blindspot is honestly documented in code comments — NOT introduced or worsened by this change.

**Council follow-up gaps closed inline (single-commit gap-closure)**

- **Case-insensitive flag parsing** (quality-reviewer F1). `OMC_AGENT_FIRST_GATE=ON`, `agent_first_gate=On`, and other uppercase / mixed-case values previously fell silently to default-off because the conf-parser regex `^(on|off)$` and the in-script `[[ "${OMC_AGENT_FIRST_GATE:-off}" == "on" ]]` comparisons were case-sensitive. Env capture in `common.sh` and `_parse_conf_file` now `tr '[:upper:]' '[:lower:]'`-normalize the value before validation; comparisons stay simple. Pinned by new `T_aff_case_env_upper / _env_mixed / _conf_upper / _env_off_upper` regression tests in `tests/test-pretool-intent-guard.sh`.
- **`agent_first_gate_state` stamp at mutation time** (data-lens P0). Both writers (`pretool-intent-guard.sh _record_first_mutation_attempt`, `mark-edit.sh _record_first_mutation_from_edit`) now stamp the *gate state AT mutation time* alongside `first_mutation_ts` and `first_mutation_tool`. Closes the schema gap that previously made opt-in vs opt-out outcomes unjoinable per-row in `/ulw-report`. Pinned by `T_aff_off_e_state` (off-state stamp under default-off) and `T_aff_on_state` (on-state stamp under explicit opt-in).
- **Stop-backstop reads state-at-mutation, not live env var** (SRE-lens P1b / abstraction-critic F2). The prior implementation acknowledged the off→on flag-flip mid-session footgun honestly in a comment but didn't close it. New code reads `agent_first_gate_state` from session state and only fires when that recorded value is `on`. Flag toggling mid-session no longer creates phantom backstop blocks on legitimately-allowed prior mutations. Closes the toggle edge case the `abstraction-critic` Finding F2 sharpened.
- **Project-conf security-flag deny-list** (security-lens P1). The conf walk-up that allows project-level `.claude/oh-my-claude.conf` to override user defaults previously let a malicious or unfamiliar repo committed conf disable `pretool_intent_guard`, `bg_spawn_gate`, `agent_first_gate`, or `no_defer_mode`. `_parse_conf_file` now accepts a `level` argument (`user`|`project`); `load_conf` passes `"project"` for walk-up paths. Security-load-bearing flags are **silently** refused when level=project (no stderr warning — an earlier draft emitted one and contaminated `test-timing.sh` T28/T29 and similar fixtures that capture combined stdout/stderr from hook scripts, because tests with `TEST_HOME=mktemp` walk PWD up into the real user's `~/.claude/oh-my-claude.conf` which gets mis-classified as "project"). User-conf and env still work normally. Behavior documented in `docs/customization.md` "Project-conf security restriction" section so users observing a project-conf entry has no effect can find the explanation. Pinned by `T_aff_project_conf_security` (denial of the override) and `T_aff_project_conf_allowed_flag` (non-security flag still applies) plus umbrella F-013 in `tests/test-stop-guard-bypass-surface.sh`.
- **Watchdog-health alarm: emit-then-stamp** (SRE-lens P1a). `bundle/dot-claude/quality-pack/scripts/session-start-watchdog-health.sh` previously stamped `watchdog_health_emitted=1` BEFORE printf — if printf failed (closed stdout, SIGPIPE, mid-hook timeout), the alarm was silently lost AND the stamp blocked the next SessionStart from re-showing. New order: print first, record gate event, stamp last. Closes the 24/7 silent-loss failure mode the alarm itself was designed to surface.
- **Stop-guard umbrella regression net for the new flag** (excellence-reviewer F1). `tests/test-stop-guard-bypass-surface.sh` had zero references to `OMC_AGENT_FIRST_GATE` even though `stop-guard.sh:311` was modified — the Coordination Rule on umbrella+per-surface lockstep was violated silently. New F-012 section adds 5 assertions: state-at-mutation=off silent, state-at-mutation=on fires, state-at-mutation=on+specialist silent, lockstep grep on `stop-guard.sh` reading the state field (not the env var), and lockstep grep on both writers stamping the field.
- **`omc-config` flag-row trim** (excellence-reviewer F2). The `agent_first_gate` row was ~140 words (5-10× longer than sibling gate rows) and would render poorly in the multi-choice UI. Trimmed to one sentence with a "see conf.example for full rationale" pointer; the long rationale lives in `oh-my-claude.conf.example` and `docs/customization.md` unchanged.
- **README + AGENTS.md surface the default flip** (excellence-reviewer F3). The "Agent-first execution" section in `README.md` now states the v1.43+ opt-in default with the three-premise WHY and points at the model-robustness Mechanism 2 doctrine. `AGENTS.md` "Protected Design Decisions" adds rows for the agent-first gate (do not re-mandate without verifying premises hold) and the security-flag project-conf deny-list (how to add a new security-load-bearing flag).
- **T_aff_off_e tool-name + T_aff_precedence_b** (quality-reviewer F2/F3). The original `T_aff_off_e` only verified `first_mutation_ts` was recorded; a refactor that dropped the tool name would have passed silently. New `T_aff_off_e_tool` asserts `first_mutation_tool == "Edit"`. `T_aff_precedence` covered env-wins-over-conf; new `T_aff_precedence_b` pins the other direction (env-empty + conf-set=on must opt in).
- **Test isolation in `test-stop-guard-bypass-surface.sh`**. Pre-existing pollution surfaced when the new security-flag restriction logged the user's real conf being mis-classified as project during test sub-shells. Added `cd "${TEST_HOME}"` in `setup()` and `cd "${_ORIG_PWD}"` in `teardown()` so the conf walk-up no longer reaches the real user conf during tests. Independent improvement, but bundled here because the new security restriction made the existing pollution visible.

**Coordination triple-write + lockstep** verified post-edit by `tests/test-coordination-rules.sh` (138/0): conf flag at `common.sh` `_parse_conf_file` parser, `oh-my-claude.conf.example` documented entry, `omc-config.sh` `emit_known_flags` row. New regression tests pinned: `tests/test-pretool-intent-guard.sh` 152/0; `tests/test-stop-guard-bypass-surface.sh` 122/0.

Files changed: `bundle/dot-claude/skills/autowork/scripts/common.sh` (parser case-fold + level arg + security deny-list + case-fold env capture), `bundle/dot-claude/skills/autowork/scripts/pretool-intent-guard.sh` (case-folded comparison + `agent_first_gate_state` stamp + Serendipity quote-strip + opt-in block wrap), `bundle/dot-claude/skills/autowork/scripts/stop-guard.sh` (read state-at-mutation), `bundle/dot-claude/skills/autowork/scripts/mark-edit.sh` (`agent_first_gate_state` stamp parallel to PreTool), `bundle/dot-claude/skills/autowork/scripts/omc-config.sh` (`emit_known_flags` trimmed row), `bundle/dot-claude/oh-my-claude.conf.example` (documented flag entry), `bundle/dot-claude/quality-pack/scripts/session-start-watchdog-health.sh` (emit-then-stamp), `tests/test-pretool-intent-guard.sh` (T_aff_off_e_tool, _e_state, _on_state, _precedence_b, _case_*, _project_conf_*), `tests/test-stop-guard-bypass-surface.sh` (F-012 agent-first opt-in + F-013 project-conf security restriction + test-isolation cd), `tests/test-e2e-hook-sequence.sh` (`setup_test` exports the flag), `docs/customization.md` (flag table row), `README.md` (agent-first section default-flip note), `AGENTS.md` ("Protected Design Decisions" entries 5 + 6).

### Anti-Pattern marker: conservative-incrementalism-when-reconstruction-is-warranted (v1.43-pre)

**Names the conservative-bias-on-improvement failure mode the user identified.** User-reported shape (2026-05-23): *"This workflow, when doing improvement, is too conservative. It should have the spirit of risk-taking. Currently, when even users have the project in a clear worse status, it doesn't have the bold to reconstruct it or improvement due to so called high risk. Always prioritize the output quality."* When a project is in clearly-worse state, the agent leans on "high risk" / "too aggressive" / "too sweeping" as generic stop signals to ship insufficient improvements. Two failure shapes: **band-aid-on-rot** (under-interpret — thin patch on a structural problem) and **defer-the-reconstruction** (over-interpret — recommend a "separate refactor pass" because reconstruction "would be too sweeping").

- **Shipped form (smaller-tier fix per `abstraction-critic` VERDICT: BLOCK on the original contract proposal).** Four edits:
  - **One new entry in `core.md` "Anti-Patterns"** (co-located with the criticism-as-literal-directive marker) naming the failure and pointing at the existing mechanisms — no new interpretive doctrine layer.
  - **Conditional rewrite of `core.md` line 36** (the "Prefer small, testable, incremental changes" rule) — chunk size now conditioned on project state, not absolute. Verification rigor stays constant across chunk sizes; chunk size is the tool, output quality is the value. Cross-references the FORBIDDEN entry for discoverability.
  - **`prompt-intent-router.sh:1271`** (mature-project directive) — removed the absolute "Avoid suggestions that imply 'rewrite this'" bias; restored sibling-consistent "Bias advisory framing toward..." opener; added chunk-size-by-project-state framing.
  - **`prompt-intent-router.sh:1314`** (coding-domain Discipline) — same conditional rebalance after the "Make changes incrementally" anchor (preserved as a test-session-resume.sh:202 lockstep pin). The coding-domain Discipline fires on EVERY coding-classified prompt — broader surface than the mature-project directive, surfaced by `excellence-reviewer` axis 6 as the more-impactful bias source.

- **Right-shape calibration via `abstraction-critic` (VERDICT: BLOCK 5 findings).** Original draft was a 70-line load-bearing "output-quality supremacy contract" parallel to the v1.40.0 no-defer contract — full FORBIDDEN list, regression test, CI wiring, tri-failure-mode preamble rename across 8 sites. The critic flagged it as the **exact paradigm-regress** as the killed `criticism-as-literal-directive` proposal (commit 43974ce, 2 days prior): the no-defer/depth contracts close *tool-shape* failures (option removal at 3 call sites) and *token-shape* failures (validator + stop-guard); conservative-bias is a *judgment-shape* failure with no surface to remove. The existing mechanism already catches this shape: `excellence-reviewer` axis 10 (uniform-shallow depth, `agents/excellence-reviewer.md:32`) explicitly catches *"task names N concerns, diff covers all N, but each surface is shipped at uniformly-shallow depth"* — that is the user's exact complaint shape. `abstraction-critic` IS the paradigm-fit critic for incremental-fix-on-structural-problem. Shipped form is the smaller-tier fix the critic recommended.

- **Meta-recursive consistency.** A 50-word criticism gets a 3-edit response, not a 70-line contract + 8-site rename + regression test + CI wiring. The proposal failing its own calibration prompt was itself evidence of the failure mode being addressed (sweeping-refactor-from-narrow-criticism, per `core.md` Anti-Pattern marker 43974ce). Per `feedback_weak_defer_shortcut_concerns` and the `over-eng-when-words-suffice` failure-mode label in `project_post_release_evaluation_method.md`, the right answer to a behavioral failure is the smallest-sufficient prime, not infrastructure.

- **Reviewer chain reconciliation.** `quality-reviewer` returned FINDINGS (3) low — F1 (router tone-drift vs siblings) fixed by restoring the "Bias advisory framing toward..." opener; F2 (anti-pattern entry too long, redundant with line 36) fixed by trimming ~150 words of redundant prose; F3 (frontmatter non-standard fields) verified as a false-positive — `node_type: memory` and `originSessionId` are harness-injected fields present on every project feedback memory. `excellence-reviewer` then returned FINDINGS (5) — F1 HIGH (second router bias surface at line 1314) fixed; F5 LOW (cross-reference from line 36 to the FORBIDDEN entry) fixed; F2 MEDIUM (5 abstraction-critic findings still `pending` in `discovered_scope.jsonl` despite all 5 being shipped) addressed by `update_scope_status` calls for each id; F4 LOW (`clearly worse than baseline` is judgment-shape token) rejected — the diagnostic IS user-anchored via the explicit OR clause ("the user has named degradation OR..."); model-judgment fallback is intentional for cases where the project is degraded but the user hasn't named it explicitly this prompt; F3 MEDIUM (no regression net protects router prose) declined — same precedent as criticism-as-literal-directive (commit 43974ce, shipped without a test).

- **Explicit non-additions.** No new contract section, no FORBIDDEN-softening list, no regression test, no CI wiring, no tri-failure-mode preamble rename, no new lifecycle hook, no new classifier predicate. Per `model-robustness.md` Mechanism-First Principle and `project_v1_43_audit_paradigm_concerns.md`, the right answer to a behavioral failure is the smallest-sufficient prime when a mechanism exists — and here both `excellence-reviewer` axis 10 and `abstraction-critic` already exist.

Files changed: `bundle/dot-claude/quality-pack/memory/core.md` (1 anti-pattern entry + line 36 rewrite with FORBIDDEN cross-reference), `bundle/dot-claude/quality-pack/scripts/prompt-intent-router.sh` (rebalances at lines 1271 + 1314).
