#!/usr/bin/env bash
#
# tools/audit-published-release-titles.sh — batch-audit published GitHub
# release titles against the canonical helper output.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
VERIFY_HELPER="${SCRIPT_DIR}/verify-published-release-title.sh"
...
usage() {
  cat <<'EOF'
Usage: bash tools/audit-published-release-titles.sh [--repo <owner/name>] [--limit <N>] [--tag <vX.Y.Z>]... [--fix]

Audits published GitHub release titles in batch by calling
tools/verify-published-release-title.sh for each selected tag.
...
Options:
  ...
  --fix                Repair drifting published release titles in place.
EOF
}

err() { printf 'audit-published-release-titles: %s\n' "$1" >&2; exit 1; }
note() { printf 'audit-published-release-titles: %s\n' "$1"; }
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      ;;
    --*)
      printf 'audit-published-release-titles: unknown arg: %s\n' "$1" >&2
      usage >&2
      exit 2
      ;;
    *)
      printf 'audit-published-release-titles: unexpected positional arg: %s\n' "$1" >&2
      usage >&2
      exit 2
      ;;
  esac
