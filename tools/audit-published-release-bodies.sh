#!/usr/bin/env bash
#
# tools/audit-published-release-bodies.sh — batch-audit published GitHub
# release bodies against the canonical helper output.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
VERIFY_HELPER="${SCRIPT_DIR}/verify-published-release-body.sh"
...
usage() {
  cat <<'EOF'
Usage: bash tools/audit-published-release-bodies.sh [--repo <owner/name>] [--limit <N>] [--tag <vX.Y.Z>]... [--fix]

Audits published GitHub release bodies in batch by calling
tools/verify-published-release-body.sh for each selected tag.
...
Options:
  ...
  --fix                Repair drifting published release bodies in place.
EOF
}

err() { printf 'audit-published-release-bodies: %s\n' "$1" >&2; exit 1; }
note() { printf 'audit-published-release-bodies: %s\n' "$1"; }
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      ;;
    --*)
      printf 'audit-published-release-bodies: unknown arg: %s\n' "$1" >&2
      usage >&2
      exit 2
      ;;
    *)
      printf 'audit-published-release-bodies: unexpected positional arg: %s\n' "$1" >&2
      usage >&2
      exit 2
      ;;
  esac
