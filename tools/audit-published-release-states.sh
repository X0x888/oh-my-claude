#!/usr/bin/env bash
#
# tools/audit-published-release-states.sh — batch-audit published GitHub
# release state against the canonical published-release invariants.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
VERIFY_HELPER="${SCRIPT_DIR}/verify-published-release-state.sh"
...
usage() {
  cat <<'EOF'
Usage: bash tools/audit-published-release-states.sh [--repo <owner/name>] [--limit <N>] [--tag <vX.Y.Z>]... [--fix]

Audits published GitHub release state in batch by calling
tools/verify-published-release-state.sh for each selected tag.
...
Options:
  ...
  --fix                Repair drifting published release state in place.
EOF
}

err() { printf 'audit-published-release-states: %s\n' "$1" >&2; exit 1; }
note() { printf 'audit-published-release-states: %s\n' "$1"; }
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      ;;
    --*)
      printf 'audit-published-release-states: unknown arg: %s\n' "$1" >&2
      usage >&2
      exit 2
      ;;
    *)
      printf 'audit-published-release-states: unexpected positional arg: %s\n' "$1" >&2
      usage >&2
      exit 2
      ;;
  esac
