...
- **Review agents cannot write; builder agents edit under your permission model.** The 26 advisory/planning/review specialists carry `disallowedTools: Write, Edit, MultiEdit` — the agents that judge work are structurally unable to alter it. The 11 domain builders (`frontend-developer`, `backend-api-developer`, the `ios-*` family, `fullstack-feature-builder`, `devops-infrastructure-engineer`, `test-automation-engineer`, `research-data-analyst`, `atlas`) can edit; their tool calls run under the same Claude Code permission prompts as the main thread.
