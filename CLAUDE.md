...
- Exemplifying directive (`exemplifying_directive` in `oh-my-claude.conf`, default `on`, v1.23.0). When ON, the prompt-intent-router injects an `EXEMPLIFYING SCOPE DETECTED` directive on fresh-execution prompts where `is_exemplifying_request` matches example markers (`for instance`, `e.g.`, `i.e.`, `for example`, `such as`, `as needed`, `as appropriate`, `similar to`, `including but not limited to`, `things like`, `stuff like`, `examples include|are|of`). The directive instructs the model to treat the example as one item of a class and enumerate sibling items rather than dropping the class. Default ON because it's informational rather than blocking, and the failure mode it defends against (under-interpretation) was the primary v1.22.x complaint. Symmetric to (but opposite of) the existing `prometheus_suggest` / `intent_verify_directive` narrowing directives — fires INDEPENDENTLY because narrowing and widening are orthogonal axes. Env: `OMC_EXEMPLIFYING_DIRECTIVE`. Telemetry: emits `gate=bias-defense` `event=directive_fired` rows with `directive=exemplifying`; surfaced in `/ulw-report` "Bias-defense directives fired" section.
