...
- **Stop orchestration and output schema.** Claude Code runs all matching handlers for one event in parallel, so managed Stop behavior MUST remain behind the single `stop-dispatch.sh` settings entry; array order is not sequencing. The dispatcher owns stdout and calls guard → timing → canary → archive deterministically. Current Claude Code accepts `hookSpecificOutput.additionalContext` from Stop/SubagentStop as non-error continuation feedback, while older clients dropped it: `stop-dispatch.sh` version-gates that model-only path and falls back to `decision: "block"` + `reason`. Keep user-visible receipts/cards on top-level `systemMessage`. `PostToolBatch` preflight must inject context only (never stop the loop), and `MessageDisplay` transformations must stay display-only. Regression nets: `tests/test-stop-dispatch.sh`, `tests/test-closeout-preflight.sh`, and `tests/test-timing.sh` T29 (visible card schema only).
